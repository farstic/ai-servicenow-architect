...
**1 — Subscription / fulfiller impact.** Does the design create or widen a role that grants *write/fulfil* access? Fulfiller-type access generally requires a paid subscription; **and where an organisation has never manually allocated user-based subscriptions, entitlements are assigned automatically from user roles and ACL access** — so granting the role IS the allocation, with no separate step for anyone to review. *(citation: `markdown/platform-administration/managing-user-subscriptions-v2.md`)* requester/approver/read paths usually do not. Count the affected user population and call out the delta. *(citation: `exploring-subscription-management-v2.md`, `markdown/platform-administration/managing-user-subscriptions-v2.md`)*
...
**4 — Now Assist / AI consumption.** AI capabilities draw on a separate AI entitlement and consume **Assists** per invocation. **The AI-native SKU has its own tier names — Foundation, Advanced, Prime — not the Standard/Pro/Enterprise vocabulary of checklist 2**, so do not answer an AI tier question in the wrong currency. **AI Control Tower is included in every one of those tiers with the same capabilities**, which settles the common question early: governance is not a tier upsell. *(citation: `markdown/intelligent-experiences/ai-native-sku-overview.md`)* Estimate volume × Assists-per-call and flag whether the AI SKU is owned. *(citation: `markdown/it-asset-management/software-asset-management/c_SAMOverview.md`)*
