...
class TaskManager {
    ...
    parseMessage(message) {
        ...
        if (text.includes('incident') && (text.includes('create') || text.includes('new')))
            return { toolName: 'snow_inc_incident_add', toolArgs: {} };
        if (text.includes('incident') && text.includes('list'))
            return { toolName: 'snow_core_records_query', toolArgs: { table: 'incident', query: 'active=true', limit: 20 } };
        if (text.includes('change') && text.includes('list'))
            return { toolName: 'snow_chg_change_requests_index', toolArgs: {} };
        if (text.includes('knowledge') && text.includes('search'))
            return { toolName: 'snow_kb_knowledge_query', toolArgs: { query: text } };
        if (text.includes('cmdb') && text.includes('health'))
            return { toolName: 'snow_core_health_dashboard_read', toolArgs: {} };
