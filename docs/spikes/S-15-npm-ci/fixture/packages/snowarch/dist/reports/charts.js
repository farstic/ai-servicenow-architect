...
export function severityPieChart(counts, size = 200) {
    ...
    for (const sev of SEVERITY_ORDER) {
        ...
        paths.push(`<path d="M ${cx} ${cy} L ${x1} ${y1} A ${radius} ${radius} 0 ${largeArc} 1 ${x2} ${y2} Z" fill="${color}" stroke="white" stroke-width="1.5"/>`);
        ...
        if (pct >= 5) {
            labels.push(`<text x="${lx}" y="${ly}" text-anchor="middle" dy="4" font-size="10" font-weight="bold" fill="white">${count}</text>`);
        }
        startAngle = endAngle;
    }
    // Legend
...
export function categoryBarChart(breakdown, width = 300, barHeight = 22) {
