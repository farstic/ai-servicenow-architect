...
export function buildRecordLink(instanceUrl, table, sysId) {
...
export function buildRecordNumberLink(instanceUrl, recordNumber) {
 ...
 */
export function findAllEntityMatches(text, instanceUrl) {
    const matches = [];
    ...
    for (const m of text.matchAll(RECORD_NUMBER_RE)) {
        ...
        if (link)
            matches.push({ start: m.index, end: m.index + m[0].length, text: m[0], link });
    }
 ...
 */
export function linkifyTextPdf(text, instanceUrl, baseStyle = {}) {
 ...
 */
export function linkifyTextPptx(text, instanceUrl, baseOpts = {}) {
    const matches = findAllEntityMatches(text, instanceUrl);
    if (matches.length === 0)
        return [{ text, options: baseOpts }];
    const parts = [];
    let lastIdx = 0;
    for (const match of matches) {
        if (match.start > lastIdx) {
