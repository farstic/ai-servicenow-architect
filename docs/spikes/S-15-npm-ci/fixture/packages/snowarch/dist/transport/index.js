 ...
 */
export async function connectTransport(server, toolCount, createServerFn) {
 ...
 */
async function setupSseTransport(server, httpServer) {
 ...
 */
async function setupStreamableHttpTransport(createServerFn, httpServer) {
    ...
    httpServer.post('/mcp', async (req, res) => {
        ...
        if (!transport) {
            ...
            const created = new StreamableHTTPServerTransport({
                ...
                allowedHosts,
                allowedOrigins,
                onsessioninitialized: (id) => { transports.set(id, created); logger.info(`MCP session initialized: ${id}`); },
            });
            created.onclose = () => { if (created.sessionId) {
                transports.delete(created.sessionId);
                logger.info(`MCP session closed: ${created.sessionId}`);
            } };
...
export function getOriginPolicy() {
    ...
    const hosts = new Set([`localhost:${port}`, `127.0.0.1:${port}`, `[::1]:${port}`, `${host}:${port}`]);
...
function addHealthRoute(httpServer) {
