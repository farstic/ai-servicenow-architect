...
const CLI_VERSION = __pkgJson.version;
...
program
    ...
    const forward = (sig) => { if (!child.killed)
        child.kill(sig); };
                ...
                : inst.environment === 'development' ? success(' DEV  ')
                    : inst.environment === 'test' ? chalk.hex('#FF6B35')(' TEST ')
                        : inst.environment === 'staging' ? navy(' STG  ')
                            : dim(' PDI  '))
            : '';
        console.log(`  ${teal(inst.name.padEnd(16))} ${bright(inst.instanceUrl.padEnd(36))} ${dim(inst.authMethod)}${envBadge ? ' ' + envBadge : ''}`);
        if (inst.group) {
...
program
    ...
    const categories = ['scan', 'review', 'build', 'ops', 'docs'];
    const labels = {
        scan: 'Scan & Monitor',
        review: 'Review & Audit',
...
program
    ...
    const provider = (options.provider || process.env.LLM_PROVIDER || storedProvider || 'anthropic');
...
program
    ...
    if (options.apiKey)
        args.push('-k', options.apiKey);
    if (options.output)
        args.push('-o', options.output);
    if (options.table)
        args.push('-t', options.table);
    if (options.scope)
