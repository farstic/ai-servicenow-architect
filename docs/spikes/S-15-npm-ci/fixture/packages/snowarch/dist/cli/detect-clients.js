...
function appExists(macPath, winExe, linuxBin) {
...
export function detectClients() {
    ...
    return clients.map(c => {
        if (c.id === 'claude-desktop') {
            return {
                ...c,
                detected: existsSync(c.configPath) ||
                    appExists('/Applications/Claude.app', join('AnthropicClaude', 'claude.exe'), 'claude-desktop'),
            };
        }
        if (c.id === 'cursor') {
            return {
                ...c,
                detected: existsSync(c.configPath) ||
                    appExists('/Applications/Cursor.app', join('Programs', 'cursor', 'Cursor.exe'), 'cursor'),
            };
        }
        if (c.id === 'vscode') {
            return {
                ...c,
                detected: appExists('/Applications/Visual Studio Code.app', join('Programs', 'Microsoft VS Code', 'Code.exe'), 'code'),
            };
        }
        if (c.id === 'windsurf') {
            return {
