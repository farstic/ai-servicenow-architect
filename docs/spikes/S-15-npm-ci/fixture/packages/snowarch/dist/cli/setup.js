...
// ─── Box drawing helpers ──────────────────────────────────────────────────────
function box(lines, color = brand) {
...
function extractFetchError(error) {
    ...
    if (cause) {
        ...
        if (code === 'ENOTFOUND')
            return `DNS lookup failed — hostname not found. Check the instance name.`;
        if (code === 'ECONNREFUSED')
            return `Connection refused — the instance may be down or blocking access.`;
        if (code === 'ECONNRESET')
            return `Connection reset by the server. Check firewall or VPN settings.`;
        if (code === 'ETIMEDOUT')
            return `Connection timed out. Check network connectivity.`;
        if (code === 'CERT_HAS_EXPIRED')
            return `SSL certificate has expired on the ServiceNow instance.`;
        if (code === 'UNABLE_TO_VERIFY_LEAF_SIGNATURE')
            return `SSL certificate verification failed.`;
        if (code)
            return `Network error: ${code} — ${cause.message}`;
 ...
 */
async function ensureGlobalCommand() {
    if (isCommandAvailable('servicenow-mcp'))
        return;
    const spinner = ora({
...
async function fetchAnthropicModels(apiKey) {
    try {
        ...
        const models = (data.data || [])
            .map((m) => m.id)
            .filter((id) => /^claude-/.test(id) && !/-\d{8}$/.test(id))
            .sort();
        return models;
    }
    catch {
...
async function fetchOpenAIModels(apiKey) {
    try {
        ...
        const models = (data.data || [])
            .map((m) => m.id)
            .filter((id) => /^gpt-/.test(id) && !id.includes('instruct') && !id.includes('realtime') && !id.includes('audio') && !id.includes('search'))
            .sort()
            .reverse();
        return models;
    }
...
export async function runSetup(options = {}) {
    ...
    console.log('');
    const toolPackage = await select({
    ...
    if (sdkEnabled)
        console.log(`  ${success('✓')} TypeScript SDK — import ServiceNow MCP Toolkit in your project`);
    if (apexEnabled)
    ...
    for (const id of chosen) {
        ...
        if (result.success) {
            ...
            if (client.note)
                console.log(`    ${dim('→')} ${subtle(client.note)}`);
        }
        else {
