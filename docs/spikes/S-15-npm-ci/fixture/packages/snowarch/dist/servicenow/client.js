...
export class ServiceNowClient {
    ...
    async executeScript(script, scope) {
        ...
        try {
            ...
            const response = await this.request(`${this.baseUrl}/api/now/v1/batch`, {
                method: 'POST',
                body: JSON.stringify({
            ...
            if (error instanceof ServiceNowError)
                throw error;
            throw new ServiceNowError(`Script execution failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'SCRIPT_FAILED');
        }
    }
    /**
