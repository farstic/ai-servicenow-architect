...
export function performanceToolManifest() {
    return [
        ...
        // ── Data Quality ─────────────────────────────────────────────────────────
        {
            ...
            description: 'Analyze data quality and field completeness for a ServiceNow table — ' +
...
export async function dispatchPerformanceAction(client, name, args) {
    switch (name) {
        // ── PA Indicators ────────────────────────────────────────────────────────
        ...
        case 'snow_perf_pa_indicator_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`PA indicator not found: ${args.sys_id_or_name}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_perf_pa_scorecard_read': {
            if (!args.indicator_sys_id)
                throw new ServiceNowError('indicator_sys_id is required', 'INVALID_REQUEST');
            // Query pa_scores for the indicator's latest data
            const scoreParts = [`indicator=${args.indicator_sys_id}`];
            if (args.breakdown_sys_id)
                scoreParts.push(`breakdown_element=${args.breakdown_sys_id}`);
        ...
        case 'snow_perf_pa_time_series_read': {
            ...
            if (args.start_date)
                parts.push(`date>=${args.start_date}`);
            if (args.end_date)
                parts.push(`date<=${args.end_date}`);
            return await client.queryRecords({
                table: 'pa_scores',
                query: parts.join('^'),
        ...
        case 'snow_perf_pa_dashboard_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`PA dashboard not found: ${args.sys_id_or_name}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_perf_homepages_index': {
            const parts = [];
            if (args.query)
                parts.push(`titleCONTAINS${args.query}`);
            return await client.queryRecords({
                table: 'sys_ui_hp',
                query: parts.join('^') || undefined,
                limit: args.limit ?? 25,
        ...
        case 'snow_perf_table_completeness_check': {
            ...
            const fieldList = args.fields.split(',').map((f) => f.trim()).filter(Boolean);
            ...
            for (const field of fieldList) {
                const nonEmpty = resp.records.filter((r) => {
                    const val = r[field];
