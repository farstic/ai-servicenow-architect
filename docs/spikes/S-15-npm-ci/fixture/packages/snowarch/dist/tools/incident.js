...
export async function dispatchIncidentAction(client, name, args) {
    switch (name) {
        ...
        case 'snow_inc_incident_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Incident not found: ${args.number_or_sysid}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_inc_incident_modify': {
            requireWrite();
        ...
        case 'snow_inc_incident_resolve': {
            requireWrite();
        ...
        case 'snow_inc_work_note_annotate': {
            requireWrite();
            if (!args.table || !args.sys_id || !args.note)
                throw new ServiceNowError('table, sys_id, and note are required', 'INVALID_REQUEST');
        ...
        case 'snow_inc_comment_annotate': {
            requireWrite();
            if (!args.table || !args.sys_id || !args.comment)
                throw new ServiceNowError('table, sys_id, and comment are required', 'INVALID_REQUEST');
