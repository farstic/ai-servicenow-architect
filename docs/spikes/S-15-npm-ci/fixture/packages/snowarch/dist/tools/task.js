...
export async function dispatchTaskAction(client, name, args) {
    switch (name) {
        case 'snow_tsk_task_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Task not found: ${args.number_or_sysid}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_tsk_task_modify': {
            requireWrite();
        ...
        case 'snow_tsk_my_tasks_index': {
            ...
            if (username)
                query += `^assigned_to.user_name=${username}`;
