...
export function catalogToolManifest() {
    return [
        ...
        // Approval tools
        {
            ...
            description: 'Create an approval rule that automatically generates approval requests when a record matches given conditions (requires WRITE_ENABLED=true). ' +
...
export async function dispatchCatalogAction(client, name, args) {
    switch (name) {
        case 'snow_cat_catalog_items_index': {
            let query = 'active=true';
            if (args.category)
                query += `^category.title=${args.category}^ORcategory=${args.category}`;
        ...
        case 'snow_cat_catalog_item_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Catalog item not found: ${args.sys_id_or_name}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_cat_catalog_item_add': {
            requireWrite();
        ...
        case 'snow_cat_catalog_item_modify': {
            requireWrite();
        ...
        case 'snow_cat_approval_rule_add': {
            requireWrite();
        ...
        case 'snow_cat_approvals_index': {
            let query = args.query || '';
            if (args.state)
                query = query ? `${query}^state=${args.state}` : `state=${args.state}`;
            const resp = await client.queryRecords({ table: 'sysapproval_approver', query: query || undefined, limit: args.limit || 10 });
            return { count: resp.count, approvals: resp.records };
        }
        case 'snow_cat_request_approve': {
            requireWrite();
            if (!args.sys_id)
                throw new ServiceNowError('sys_id is required', 'INVALID_REQUEST');
        ...
        case 'snow_cat_request_reject': {
            requireWrite();
            if (!args.sys_id || !args.comments)
                throw new ServiceNowError('sys_id and comments are required', 'INVALID_REQUEST');
        ...
        case 'snow_cat_sla_details_read': {
            if (!args.task_sys_id)
                throw new ServiceNowError('task_sys_id is required', 'INVALID_REQUEST');
        ...
        case 'snow_cat_active_slas_index': {
        ...
        case 'snow_cat_catalog_variable_add': {
            requireWrite();
        ...
        case 'snow_cat_catalog_ui_policy_add': {
            requireWrite();
