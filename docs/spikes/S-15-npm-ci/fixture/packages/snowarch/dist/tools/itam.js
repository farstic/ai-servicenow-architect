...
export async function dispatchItamAction(client, name, args) {
    switch (name) {
        case 'snow_itam_assets_index': {
            let query = '';
            if (args.state)
                query = `install_status=${args.state}`;
            if (args.assigned_to)
                query = query ? `${query}^assigned_to=${args.assigned_to}` : `assigned_to=${args.assigned_to}`;
            if (args.location)
                query = query ? `${query}^locationLIKE${args.location}` : `locationLIKE${args.location}`;
            if (args.query)
        ...
        case 'snow_itam_asset_add': {
            if (!args.display_name)
                throw new ServiceNowError('display_name is required', 'INVALID_REQUEST');
            requireWrite();
            const payload = { display_name: args.display_name };
        ...
        case 'snow_itam_asset_retire': {
            if (!args.sys_id)
                throw new ServiceNowError('sys_id is required', 'INVALID_REQUEST');
            requireWrite();
            const payload = { install_status: 'retired' };
            if (args.disposal_reason)
                payload.disposal_reason = args.disposal_reason;
            if (args.disposal_date)
                payload.disposal_date = args.disposal_date;
            const result = await client.updateRecord('alm_asset', args.sys_id, payload);
            return { action: 'retired', sys_id: args.sys_id, ...result };
        }
        case 'snow_itam_software_licenses_index': {
            const resp = await client.queryRecords({
                table: 'alm_license',
        ...
        case 'snow_itam_license_compliance_read': {
            ...
            const summary = resp.records.map((r) => ({
        ...
        case 'snow_itam_asset_contracts_index': {
            let query = args.active !== false ? 'active=true' : '';
            if (args.asset_sys_id)
                query = query ? `${query}^asset=${args.asset_sys_id}` : `asset=${args.asset_sys_id}`;
            const resp = await client.queryRecords({
                table: 'ast_contract',
                query: query || undefined,
                limit: args.limit || 25,
        ...
        case 'snow_itam_asset_lifecycle_track': {
            ...
            if (!/^[0-9a-f]{32}$/i.test(args.asset_id)) {
                ...
                if (resp.count === 0)
                    throw new ServiceNowError(`Asset not found: ${args.asset_id}`, 'NOT_FOUND');
        ...
        case 'snow_itam_license_optimization_read': {
            const threshold = args.threshold_pct || 80;
            let query = '';
            if (args.software_name)
                query = `display_nameLIKE${args.software_name}`;
            const resp = await client.queryRecords({
                table: 'alm_license',
                query: query || undefined,
                limit: 100,
            ...
            const recommendations = resp.records.map((r) => {
                ...
                if (usagePct < threshold)
                    recommendation = 'underutilized — consider reducing licenses';
                if (inUse > total)
                    recommendation = 'over-allocated — purchase additional licenses';
                return {
                    license: r.display_name,
                    product: r.product,
                    total,
            ...
            const underutilized = recommendations.filter((r) => r.recommendation.startsWith('underutilized'));
            const overAllocated = recommendations.filter((r) => r.recommendation.startsWith('over-allocated'));
            return {
