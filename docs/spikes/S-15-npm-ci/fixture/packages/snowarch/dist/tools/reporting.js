...
export async function dispatchReportingAction(client, name, args) {
    switch (name) {
        case 'snow_rpt_reports_index': {
            // Latest release: /api/now/reporting supports sysparm_contains for name search
            let query = '';
            if (args.search)
                query = `nameLIKE${args.search}`;
            if (args.category)
                query = query ? `${query}^categoryLIKE${args.category}` : `categoryLIKE${args.category}`;
        ...
        case 'snow_rpt_report_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Report not found: ${args.sys_id_or_name}`, 'NOT_FOUND');
        ...
        case 'snow_rpt_scheduled_job_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Scheduled job not found: ${args.sys_id_or_name}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_rpt_scheduled_job_add': {
            ...
            if (args.run_period)
                data.run_period = args.run_period;
        ...
        case 'snow_rpt_job_run_history_index': {
            const parts = [];
            if (args.job_sys_id)
                parts.push(`sysauto=${args.job_sys_id}`);
            if (args.status)
