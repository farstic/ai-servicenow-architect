...
export async function dispatchDevopsAction(client, name, args) {
    switch (name) {
        case 'snow_devops_devops_pipelines_index': {
        ...
        case 'snow_devops_devops_pipeline_read': {
            if (!args.sys_id)
        ...
        case 'snow_devops_deployments_index': {
            let query = '';
            if (args.pipeline_sys_id)
                query = `pipeline=${args.pipeline_sys_id}`;
            if (args.environment)
                query = query ? `${query}^stage=${args.environment}` : `stage=${args.environment}`;
            if (args.state)
                query = query ? `${query}^status=${args.state}` : `status=${args.state}`;
            const resp = await client.queryRecords({
        ...
        case 'snow_devops_deployment_read': {
            if (!args.sys_id)
                throw new ServiceNowError('sys_id is required', 'INVALID_REQUEST');
        ...
        case 'snow_devops_deployment_track': {
            ...
            if (args.notes)
                payload.description = args.notes;
        ...
        case 'snow_devops_devops_insights_read': {
            ...
            const success = resp.records.filter((r) => r.status === 'success').length;
            const failed = resp.records.filter((r) => r.status === 'failed').length;
            return {
                period_days: days,
