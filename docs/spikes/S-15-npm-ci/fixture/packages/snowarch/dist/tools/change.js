...
export async function dispatchChangeAction(client, name, args) {
    switch (name) {
        case 'snow_chg_change_request_add': {
            requireWrite();
            ...
            if (args.implementation_plan)
                data.implementation_plan = args.implementation_plan;
            if (args.backout_plan)
                data.backout_plan = args.backout_plan;
            if (args.test_plan)
                data.test_plan = args.test_plan;
            if (args.cmdb_ci)
                data.cmdb_ci = args.cmdb_ci;
        ...
        case 'snow_chg_change_request_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Change request not found: ${args.number_or_sysid}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_chg_change_request_modify': {
            requireWrite();
        ...
        case 'snow_chg_change_requests_index': {
        ...
        case 'snow_chg_change_for_approval_submit': {
            requireWrite();
            if (!args.sys_id)
        ...
        case 'snow_chg_change_request_close': {
            requireWrite();
        ...
        case 'snow_chg_cab_meeting_schedule': {
            requireWrite();
            ...
            if (!/^[0-9a-f]{32}$/i.test(args.change_id)) {
                ...
                if (resp.count === 0)
                    throw new ServiceNowError(`Change request not found: ${args.change_id}`, 'NOT_FOUND');
