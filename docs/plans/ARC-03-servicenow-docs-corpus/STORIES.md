...
- Expected first output (19 lines): the areas listed in the facts block above. `now-assist` does **not** appear until a skill cites it; `now-assist` is not a directory at `ba513f2` (see S04).
...
2. Given a skill file with the citation `` `markdown/now-assist/foo.md` `` added, `--check` exits 2 and prints `vendor/docs-areas.txt is stale: +now-assist`.
