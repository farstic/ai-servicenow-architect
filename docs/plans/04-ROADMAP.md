...
| `release.yml` re-runs every gate on three OSes and publishes the GitHub Release with doctor JSON and install-metrics assets | `.github/workflows/release.yml` (tags only; the one workflow that may write) with `tests/release-workflow.test.mjs`; the rehearsal Release is the architect's, same row as above |
