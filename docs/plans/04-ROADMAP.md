...
| **M3 — Install, wizard, doctor** *(**COMPLETE 2026-09-11 — 36 of 36**; merged to `main` 2026-09-11 @ `81bff6c` (PR #123, owner-approved; 42 required contexts green), started 2026-09-09; **ARC-08 COMPLETE 2026-09-11 — all 11 stories**; **ARC-06 COMPLETE 2026-09-10 — all 14 stories**, with the Windows double-click / Ctrl-C / GPO, S-03/S-04, S12's `~/.claude.json` halves and S13's reader two and Path B recorded as owner-sitting work rather than ticked; **ARC-07 COMPLETE 2026-09-10 — all 11 stories**, with the live PDI run, the S-04 masked-input matrix, Known Folder Move, a real legacy file, the four skill passes, the password-manager examples and the E2E secrets recorded the same way)* | ARC-06 (all 14) · ARC-07 (all 11) · ARC-08 (all 11) | 36 | 57.5–91.5 (per-story); ARC totals 21.5–38.5 + 20–25 + 22–30 | G3 (S-14 before ARC-06-S01), G4 (Windows spikes before native-Windows promises), G5 | `git clone … && ./bootstrap.sh` reaches `Mode: design-only` on macOS/Linux/Windows with and without Node (13 CI bootstrap cells incl. Windows without Git Bash); `./bootstrap.sh` → wizard → `DOCTOR: … 0 fail` / `Mode: live — …` with `~/.claude.json` byte-identical; `snowarch instance add|list|test|set-preset --ack-prod|import --from-legacy` per D-04/D-05 with the per-flag review screen; `/snowarch status|setup-instance|doctor` work in-session (S-02/S-16 verdict texts applied); doctor 0 FAIL on the reference install, banner < 1 s, `--fix` idempotent, leftover detectors print exact commands; `docs/INSTALL.md`, `MODES-AND-PRESETS.md`, generated `TROUBLESHOOTING.md` |
| **M4 — Release 2.0.0** *(**COMPLETE 2026-09-11 — 11 of 11**; merged to `main` 2026-09-12 @ `16fe2df` (PR #148, owner-approved 2026-09-11; 54 required contexts green; the `v2.0.0-rc.0` rehearsal green on run 10 with both negatives refused); the `v2.0.0` tag is cut after the acceptance sittings)* | ARC-09 (all 11) | 11 | 15–25 | M3; G6; owner's npm token only for the optional S10 | `node scripts/release.mjs 2.0.0` produces one commit and tag `v2.0.0` (contract sha + docs pin in the message); `release.yml` re-runs every gate on three OSes and publishes the GitHub Release with doctor JSON and install-metrics assets; `docs/CHANGELOG.md` carries the "supersedes engine v2.8.0 and snow-mcp 1.0.0" note and the migration notes; `./snowarch version` equals the tag; `./snowarch upgrade` proven with two fixture releases and credentials byte-identical; nine CI cells + the Windows-without-Git-Bash cell green on every commit; `publish-npm.yml` dry run green (real publish is a manual dispatch) |
| **M5 — Migration and cutover** *(**In progress — 8 of 10**, started 2026-09-12; S06/S08 are post-tag sittings, S09/S10 keep a post-tag run half. Scheduled: S06 by 2026-09-26, S08 by 2026-10-03, S09/S10 on 2026-10-08)* | ARC-10 (all 10) | 10 | 10.5–14 + a fixed 14-calendar-day wait | G7 (`v2.0.0` tag) | `docs/MIGRATION.md` machine-checked against the doctor's leftover detectors; author's machine cut over (doctor 0 FAIL, legacy store and stale registrations gone); five clean-machine validation records (macOS/Ubuntu/Windows design-only, one live `pdi-developer`, one proxied laptop) committed under `docs/validation/`; deprecation notices in both old repositories with `@farstic/snow-mcp@1.0.0` untouched; two-week review held and the archive executed or deferred with a reason |
...
### M3 exit evidence — recorded 2026-09-11

Every exit condition from the M3 row, with what was measured on `arc-08/doctor` @ the ARC-08-S11 merge rather than what was intended. Nine bootstrap `node-cli` cells, three `no-node`, one `no-gitbash`; 42 required contexts.

| Exit condition | Evidence |
|---|---|
| `git clone … && ./bootstrap.sh` reaches `Mode: design-only` on macOS/Linux/Windows with and without Node (13 CI bootstrap cells incl. Windows without Git Bash) | ARC-06-S14's `bootstrap` job, 13 cells, green on every commit; `scripts/ci/assert-bootstrap.mjs` asserts the install promise in one implementation for every cell |
| `./bootstrap.sh` → wizard → `DOCTOR: … 0 fail` / `Mode: live — …` with `~/.claude.json` byte-identical | **Partly deferred.** The wizard and the doctor are proven separately in CI; the end-to-end run against a live PDI is the owner's sitting (D2/D2b, D5). `~/.claude.json` byte-identical is proven by `tests/doctor/stale-registration-e2e.test.mjs` after a plain run, `--json` and `--fix` |
| `snowarch instance add\|list\|test\|set-preset --ack-prod\|import --from-legacy` per D-04/D-05 with the per-flag review screen | ARC-07 COMPLETE (all 11 stories); `packages/snowarch/tests/cli/instance.test.ts` and the review-screen tests; the masked-input matrix is owner-sitting |
| `/snowarch status\|setup-instance\|doctor` work in-session (S-02/S-16 verdict texts applied) | ARC-08-S09 (`## status` renders `doctor --quick --json` from named keys), ARC-07-S09 (`setup-instance`, `--resume`); the four real-session runs are owner-sitting D4 |
| doctor 0 FAIL on the reference install | In CI: `DOCTOR: 26 ok, 0 warn, 1 fail (10 skip) — expected here: E-00` on all nine `node-cli` cells, the one failure being that a hosted runner has no Claude Code, asserted both ways. The reference-machine run is owner-sitting D1 |
| banner < 1 s | Five cold spawns per cell: **ubuntu 330 ms · macOS 614 ms · Windows 910 ms · Windows-without-Git-Bash 895 ms**, budget 1000 ms, in each cell's step summary. Windows has ~9% headroom — a risk carried into ARC-09 |
| `--fix` idempotent, leftover detectors print exact commands | `tests/doctor/fix.test.mjs` (the composite fixture, F1–F7, the `.mcp.json` refusal); `tests/doctor/stale-registration-e2e.test.mjs` (both `claude mcp remove` lines verbatim, on three OSes) |
| `docs/INSTALL.md`, `MODES-AND-PRESETS.md`, generated `TROUBLESHOOTING.md` | all three generated and proved current by `npm run gen:check` (10 generators) on every cell; `TROUBLESHOOTING.md` renders from the error registry — one remedy string, three documents, asserted character-identical |

Three findings from M3 that outlived their stories and are carried forward:

- **A check nobody has ever run on a platform is a check that does not work there.** E-03 could not read `npm --version` on Windows at all — `npm` is `npm.CMD`, and Node has refused to exec a batch file without a shell since CVE-2024-27980 — and it had been that way since the check was written. Nothing found it until the doctor joined the bootstrap cells in ARC-08-S11. The same run found that `--json` printed a prose note before the object whenever `.local` refused a write.
- **A batch file invoked without `call` never comes back.** `.\snowarch.cmd doctor` transferred control and the exit-code check on the next line never ran; the step's code silently became the callee's. The bootstrap job's own Windows steps have the same shape, so their trailing `if not "%ERRORLEVEL%"=="0"` lines are dead — carried into ARC-09-S08's matrix work.
- **Redaction is a chokepoint, not a discipline.** ARC-08-S11's second drill was supposed to plant an un-redacted string in a check's `detail` and watch the e2e go red. It could not: `runner.mjs` redacts every result centrally, so a check that prints the store still comes out masked. The drill had to break the mask itself. Worth knowing before someone "simplifies" the chokepoint away.
...
| Exit condition | Evidence |
|---|---|
| `node scripts/release.mjs 2.0.0` produces one commit and tag `v2.0.0` (contract sha + docs pin in the message) | The script and its refusals are proven by `tests/release.test.mjs`, `tests/version-tag.test.mjs` and the `release-dryrun` cells on three OSes every commit; end to end on the S07 harness in S11's walkthrough (`release 9.2.0 → 9.3.0`, plan printed, gates `dist → lint → test → contract → docs`). **The real cut is executed by the architect after this merge — numbers recorded in the roadmap record PR.** |
| `release.yml` re-runs every gate on three OSes and publishes the GitHub Release with doctor JSON and install-metrics assets | **Rehearsed for real, 2026-09-12:** `v2.0.0-rc.0`, run 34660381461 — `verify` ×3 and `publish` green; exactly seven assets (three `doctor-<os>.json`, three `install-metrics-<os>.json`, `install-metrics.md`); metrics macos 35.8 s · ubuntu 7.8 s · windows 37.3 s bootstrap total; the two negatives refused (`v9.9.8` lightweight → `not annotated`; `v9.9.7` edited `contract:` → `sha in message != dist/contract.json`); Release/tags/branch deleted afterwards. Runs 1–9 each found one defect no fixture or `--dry-run` could reach (C12, C12c, C16, C17, C17b, C18, C19, C20, C21), all fixed before this merge. `.github/workflows/release.yml` with `tests/release-workflow.test.mjs` |
| `docs/CHANGELOG.md` carries the "supersedes engine v2.8.0 and snow-mcp 1.0.0" note and the migration notes | `## Unreleased` → `### Notes`, five-point migration for snow-mcp 1.0.0 users, moved into its release section by the release script |
| `./snowarch version` equals the tag | `tests/version-consistency.test.mjs`; on the harness, `snowarch 9.2.0 · contract 8d9d0847b597 · docs pin …` with `tag: v9.2.0 (exact)` and `tag says: contract 8d9d…8a33` |
| `./snowarch upgrade` proven with two fixture releases and credentials byte-identical | `tests/upgrade/upgrade.e2e.test.mjs` on the two-release harness and the `upgrade-e2e` cells on three OSes; the plan names what re-runs before anything moves, and `.local/instances.json` is never read or written |
| Nine CI cells + the Windows-without-Git-Bash cell green on every commit | **54 required contexts**, green on every story merge of this arc. `bootstrap (no-gitbash, windows-latest)` keeps its name; `windows-native` adds three `cmd.exe`-only cells; `eol (ubuntu-latest)` / `eol (windows-latest)` clone with `core.autocrlf=true` |
| `publish-npm.yml` dry run green (real publish is a manual dispatch) | The workflow, the guard (`assert-publish-target.mjs`, refusing anything but `@farstic/snowarch` before `npm ci` and before the token) and the tests are merged; **the first dispatch is the owner's** — `docs/spikes/OWNER-SITTING.md` § Sitting E, `tag: v2.0.0`, `dry_run: true`, with the secret deferred to the acceptance phase by the owner's decision of 2026-09-11 |

**The banner numbers this milestone is allowed to quote** (from the `fb1e466` run, re-run raw/cost ·
fast, in ms): windows n20 290/245 · 70 · n22 285/237 · 70 · n24 274/229 · 61 · no-gitbash 291/245 ·
69 · macOS n20 148 · 43 · n22 174 · 46 · n24 113 · 38 · ubuntu n20 139 · 42 · n22 99/79 · 35 · n24
107 · 30. Before ARC-09-C8 the Windows re-run cost was **686–1014 ms** against a 1000 ms budget; C8
made `--quick` a cost contract enforced per shared context, and that is where the difference went.

**Ten chores, C1–C10**, each a defect found while building a story and fixed in the same arc — the
ARC-09 README's chore table is the record. Three of them were found by a test written for something
else: C8 by a phase table, C9 by C8's fixture change, C10 by a question about what ships in a
tarball.
...
| **G6 — Deferred spikes** | ARC-04-S14 (CHANGELOG gate-split note), M2 exit; ARC-02 listing acceptance | S-10 verdict recorded by ARC-04-S05 (`docs/spikes/S-10-readonly-preset-sufficiency/`), S-13 by ARC-02-S03 (`docs/spikes/S-13-skill-description-cap/`) | ARC-04-S05, ARC-02-S03 |
...
- [ ] **After the tag (M5, not blocking the tag):** ARC-09-S03 Release page with doctor JSON + install-metrics assets; author cutover (ARC-10-S06, **by 2026-09-26**); five clean-machine records (S08, **by 2026-10-03**); deprecation notices (S09, **2026-10-08**); two-week review and archive decision (S10, **2026-10-08**).
