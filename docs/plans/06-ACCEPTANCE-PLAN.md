...
asks for (**B05-06** read a vitest duration from a CI cell that runs `npm run contract` and never
invokes vitest); it can quote **a sentence the product never says** (**B06-02** expected "did not
answer initialize within 1 ms", where the code says `the server did not answer: <error>` and the
timeout is not `MCP_TIMEOUT` at all); it can describe **a capability neither side has** (**B06-04**
...
prints — no `2.25` exists anywhere in the tree — conflating git *absent* (`bootstrap.ps1`'s `Die 'B00' 'git not found'`, `git
...
| B00-08 | ARC-00-S14 AC6 / README R11 | Gate sign-off block with dates and owner initials; ARC-06-S01 start later than the S-14 conclusion | Block absent from the ARC-00 README; `04-ROADMAP.md` §9 row 6 marks it half open; the date ordering is satisfiable (ADR-0006 Accepted 2026-09-08 < ARC-06-S01 Done 2026-09-09) but nothing writes it down | Append the two-line sign-off block with the recorded dates (ARC-01 entry: ADR-0001/0002/0003 Accepted 2026-09-04/2026-09-07; LICENSE/NOTICE agreed 2026-09-06/07; ARC-06 entry: verdict dates, S-14 concluded 2026-09-08, Q-B "pending"); leave `initials: PENDING OWNER`; put the initialling into OWNER-SITTING.md as an explicit row | architect | S| **rework — and now closed.** Block appended with the recorded dates and the ordering it exists to enforce; the initials cell was left open on purpose, and **the owner filled it on 2026-09-16 (`CG`)** — both gate rows now carry it |
| B00-09 | ARC-00-S03 task 4 / AC1 | Owner initials each ADR (`Decision owner` row) | ADR-0001 to ADR-0008 Decision-owner rows all read `initials PENDING OWNER`; ADRs were Accepted on the architect's ratification only. **Closed 2026-09-16: the owner gave `CG` and every cell now reads `CG, 2026-09-16`** | Owner sitting row: initial the eight ADRs (edit only the Decision-owner cell); test that no `docs/decisions/ADR-*.md` with Status Accepted still contains `PENDING OWNER` (test must be written to skip or expect-fail until the sitting) | architect (row), dev (test) | S| **record — and now closed.** No expect-fail test, as ruled: one red from the day it was written until the sitting is noise. The Sitting A row was the enforcement and it fired — initials `CG, 2026-09-16`. A test exists now and is a different shape: green because the initials landed, red only if they are removed or a new uninitialled cell appears. A ratchet after the fact is not the red-by-design test that was refused |
...
| ID | Where | Criterion | Why uncovered | Proposed check | Owner | Size| Vetting |
|---|---|---|---|---|---|---|---|
| B05-01 | ARC-05-S01 AC 4 | `pin.mjs --yes` exits 1 with exactly one `REGATE snow_fluent_script_exec:` line against a fixture; `--accept-regate` rewrites gate | No test invokes pin.mjs; the story's `tests/contract/fixtures/contract-*.json` (renamed / re-gated / missing) do not exist; only drill scenario 2 in docs/CONTRIBUTING.md shows the refusal (dated manual capture) | Add `tests/contract/pin.test.mjs` spawning `node packages/contract/pin.mjs --yes` with `SNOW_CONTRACT_PATH`/`SNOW_PIN_PATH` at fixture contracts (baseline, re-gated, missing tool); assert exit 1 + single REGATE line, exit 0 after `--accept-regate`, committed pin untouched | dev | M | **rework** — nothing invoked `pin.mjs`; one REGATE line, the remedy, the pin unchanged on refusal, then `--accept-regate` applying it |
| B05-02 | ARC-05-S01 AC 5 | Required tool absent → pin.mjs exits 1 with `MISSING <name> — not in packages/snowarch/dist/contract.json`, file unchanged | Same gap; drill scenario 1 shows the server test and L08, not the pin.mjs MISSING line | Same pin.test.mjs: fixture with one required tool removed; assert the MISSING line and a byte-identical pin file afterwards | dev | (in B05-01) | **rework** — one MISSING line, and "nothing written" asserted as sha256 rather than by re-reading one field |
| B05-03 | ARC-05-S01 AC 7 (reader: unknown) | Propose → review → apply: without `--yes` prints proposal then waits; `n` exits 3; non-TTY without `--yes` exits 2 with the stdin message; Ctrl-D aborts | No test drives pin.mjs stdin (`tools/snowarch/tests/bootstrap-plan.test.mjs` "stdin closing is a quit" covers the bootstrap wizard, not pin.mjs) | pin.test.mjs: spawn with stdio pipe (non-TTY) → exit 2 and message; assert a closed stdin resolves to abort (exit 3), not exit 13 | dev | (in B05-01) | **rework** — non-TTY exits 2 with the proposal still printed first; the exit table is read out of the source, not restated |
| B05-04 | ARC-05-S02 AC 1 | After adding a key to `retired-identifiers.json`, `--check` exits 1 and prints the missing key; re-run without `--check` adds it | `tests/contract/retired-names.test.mjs` runs `--check` on the committed tree only (positive case) | Copy packages/contract to a temp root, append a key, run `gen-retired-names.mjs --check` → exit 1 naming the key; run without `--check` → key present | dev | S | **rework** — the proposed fixture dies on a missing rename map; copy-then-prove with both package directories and a marker, plus a control |
| B05-05 | ARC-05-S10 AC 1 to AC 3 (AC 3 reader: unknown) | `flagNames(loadContract()).length === 6` stdlib-only; stale pin throws `ContractPinMismatch` naming both shas, `verifyPin:false` loads; `expandPreset` throws `SCRIPTING_ENABLED requires WRITE_ENABLED` and pdi-developer expands to the §6.3 row | No loader unit test; `expandPreset`/`ContractPinMismatch` appear only in `packages/contract/lib/contract.mjs` and `tools/snowarch/lib/steps/B05.mjs`/`B06.mjs`; `tests/contract/no-literals.test.mjs` imports loadContract but asserts nothing about it; `tools/snowarch/tests/b03-b05.test.mjs` "AC 3 — one byte in the contract, and B05 names both 12-char prefixes" is indirect; story task 2 not delivered | Add `tests/contract/contract-lib.test.mjs`: flagNames length 6 on the real tree; loadContract with `SNOW_PIN_PATH` at a stale fixture throws ContractPinMismatch with both shas and loads with `verifyPin:false`; `expandPreset('custom',{SCRIPTING_ENABLED:'true'})` throws the exact string; `expandPreset('pdi-developer')` deep-equals `contract.presets['pdi-developer']` | dev | S | **rework** — no loader test existed; six flags named, both shas in the mismatch, the dependency pair named, the preset asserted whole |
| B05-06 | ARC-05-S08 AC 7 (reader: unknown) | Test 3 runs both flag states for every catalogue tool in under 30 s on the slowest CI runner | No timing assertion in `packages/snowarch/tests/contract.test.ts`; no CI step reports duration | read vitest's duration for contract.test.ts on the **`test (windows-latest, node 20)`** cell log **(corrected at ARC-05 acceptance, 2026-09-13, on 0a84406: the row named the `contract` cell, which runs `npm run contract` and never invokes vitest. A criterion pointing at a log that cannot contain its number is the fourth instance of that pattern in this acceptance pass.)** Recorded with the measurement rather than left to the RC | architect | S | **record** — the row named a cell that never invokes vitest; corrected here and in §3 row 6.1. Measured on run `34729868657` / job `103650549196`: **1,295 ms against a 30 s cap** |
| B05-07 | ARC-05-S09 AC 5 | `node scripts/contract-gate.mjs --skip-build` skips step (1) and still runs (2) to (4) | `tests/release.test.mjs` "C12b: the contract gate runs after the writes" asserts the release script invokes `contract-gate.mjs --skip-build` through a mock; nothing tests contract-gate.mjs's own step selection | Add a test running contract-gate.mjs with a fake npm on PATH (or a `--dry-run` flag) asserting the printed step list omits `dist` under `--skip-build` **(corrected at ARC-05 acceptance: it does NOT — `dist` is step 2 and still runs. The step `--skip-build` skips is step 1, id `server`, whose SUBJECT is the committed `dist/`; the proposal confused the id with what it checks, and a test written to it would have asserted the opposite of the criterion.)**; or manual-rc (step 6.2): check the `CONTRACT GATE:` line lacks `dist ok` | dev (test) or architect (run) | S | **rework** — the proposed check asked for the opposite of the criterion; both summary lines asserted in full, and §3 row 6.2 corrected with them |
...
>   The DNS behaviour is unit-tested (`tests/docs-sync.test.mjs`'s `cannot reach github.com (DNS)` rows); ARC-03-S05's AC text needs
...
| 2.3 | ADRs | `grep -n 'Status' docs/decisions/ADR-000[1-8]*.md`; `grep -n 'DECIDED 2026-09-04' docs/decisions/ADR-000[1-6]*.md` | Seven (plus ADR-0008) present; ADR-0001 to 0005, 0007 Accepted with DECIDED text; ADR-0006 "Accepted (2026-09-08) — monorepo path confirmed"; ADR-0002 Accepted 2026-09-07; ADR-0009 Accepted 2026-09-08. Decision-owner rows read `CG, 2026-09-16` (initialled at Sitting A; they read `PENDING OWNER` until then) | ARC-00 R7, R10, ARC-05-S11 |
...
| # | Check | Command | Expected | Source |
|---|---|---|---|---|
| 6.1 | Contract test duration | Read the `test (windows-latest, node 20)` cell log on the RC run for contract.test.ts **(corrected at ARC-05 acceptance, 2026-09-13, on 0a84406: this said `contract (windows-latest, node 20)`, which runs `npm run contract` and never invokes vitest — a step pointing at a log that cannot contain the number)** | Under 30 s | ARC-05-S08 AC7 (B05-06) |
| 6.2 | Skip-build gate | `node scripts/contract-gate.mjs --skip-build` | Exactly `CONTRACT GATE: server skipped · dist ok · generated ok · engine ok` **(corrected at ARC-05 acceptance, 2026-09-13, on 0a84406: this said "without `dist ok`", which is the opposite — `dist` is step 2 and still runs; the skipped step is step 1, id `server`. An RC operator following the old wording would have marked a CORRECT gate as failed. `tests/contract/contract-gate.test.mjs` asserts the gate's step SELECTION through its `--plan` mode and pins the summary line's FORMAT from the source; the line itself is produced by CI's `contract` cell on every push. The test does not run the gate: step 1 rebuilds `dist/`, and a unit test that deletes a committed directory of its own tree races every other test that reads it — see ARC-05-C3.)** | ARC-05-S09 AC5 |
...
| ~~7.3~~ | ~~Broken bootstrap.cmd~~ | **WITHDRAWN — do not run.** `bootstrap.cmd` has no goto label (a six-line wrapper), and it is invoked **eleven times across four jobs**, so "others green" cannot hold. Measured on `develop`; see B09-03 | — | ARC-09-S08 AC5 (B09-03) |
| 7.4 | No git on PATH | windows-latest branch run with `C:\Program Files\Git\cmd` also stripped | `.\bootstrap.cmd --mode design --yes` exits **3** printing **`git not found`** with remedy `winget install Git.Git` (`bootstrap.ps1`'s `Die 'B00' 'git not found'`). *Corrected per B09-02:* the old expectation `git ≥ 2.25 not found` is a literal the launcher never prints and conflates two paths — absent (:132) vs below floor (:133, `git <v> found, >= 2.34.1 required`). Pinned by `tests/launcher-parity.test.mjs` | ARC-09-S08 AC4 (B09-02) |
...
| ARC-10-S08 AC1 to AC7, ARC-10 README-AC2 | Three clean machines (macOS, Ubuntu, Windows) reach `Mode: design-only` from the install page alone; one reaches `Mode: live (pdi-developer)` against a PDI; one proxied laptop; records committed as `docs/validation/<date>-<os>.md` | README: post-tag sitting; Sittings A / C / D adjacent | **Ubuntu leg done 2026-09-17 on `v2.0.0-rc.4`** — `docs/validation/2026-09-17-ubuntu.md`: Path A followed on its own, `Mode: design-only`, doctor **28 ok / 0 warn / 0 fail**, on ubuntu 22.04.5 **arm64** with stock git 2.34.1. Four of the row's five cutover tests ran headlessly (T-02, T-07, T-10, T-20 pass; T-01 reaches its dispatch gate but not the §6.2 half in the two turns its spec scripts). **macOS and Windows legs still open**, and the Ubuntu leg carries two caveats in the record: the page's own command installs the default branch rather than the candidate (`ARC-06-C12`), and the workspace was never trusted, so nothing here exercises the allow-list |
| ARC-10-S06 AC1 to AC7, ARC-10 README-AC1, README-AC4 (v2.0.0 half) | Author's machine cutover on the current release tag following docs/MIGRATION.md: doctor 0 FAIL, no stale `~/.claude.json` entry, `~/.config/servicenow-mcp/` absent, engagement folders untracked; record `docs/validation/<date>-author-cutover.md`; flip the v2.0.0 assertion in `tests/architecture-history.test.mjs` | README: post-tag sitting | open (post-tag) |

### Sitting B

No reader assigned a criterion to Sitting B, but **Sitting B is not empty** — checked 2026-09-12:
`docs/spikes/OWNER-SITTING.md`'s *Sitting B — Design-only in a real Claude session* defines it ("Design-only in a real Claude session") and it carries
...
| ARC-04-S04 c7 | S-17 unconfigured server: CONFIRMED on 2.1.258; Ubuntu row NOT RUN | `docs/spikes/S-17-unconfigured-server/README.md` | **Ubuntu row attempted 2026-09-17 and still open — the gate is named now.** `docs/validation/2026-09-17-ubuntu.md` records it: the design-only half reproduces on Linux (`claude mcp get servicenow` → *Status: ✘ Rejected (see disabledMcpjsonServers)*), but the ENABLED-and-unconfigured half needs an **interactive approval** — removing the server from `disabledMcpjsonServers` and adding `enabledMcpjsonServers` both left it at *⏸ Pending approval (run `claude` to approve)*, and a headless session cannot answer a TUI dialog. Forging the approval flag by hand was deliberately **not** done: a CONFIRMED obtained that way is worth less than a NOT RUN. Needs one interactive Linux session — Sitting C or D. One lead to shorten it: `claude mcp list` prints a live status column, so the `/mcp` panel may not be the only way to answer the first half |
...
| ARC-07 R1, ARC-07-S05 AC1, AC2 | The wizard against a real instance: credentials typed, nothing echoed, five probes ok, store 0600/0700, six flags, toolPackage full, maxRecords 100 | "The wizard against a real instance (ARC-07-S05)" items (1) to (3) | **PASS 2026-09-19 (Sitting C §A, macOS, rc.5)** — `docs/validation/2026-09-19-macos-sitting-c.md`. Wizard against a real instance: credentials typed, nothing echoed at `[4/6]`, the five capability probes ok (plus `auth`; `fluent not installed`), store 0600 / dir 0700, six flags, `toolPackage full`, `maxRecords 100`. **A3 is NOT evidenced** — the `ps`-arguments and shell-history checks were not run, and the store says nothing about either |
...
| ARC-06 R3 (B06/B08 against an instance) | see Sitting A row; same procedure | | **PASS 2026-09-19 (Sitting C §A, macOS, rc.5)** — `docs/validation/2026-09-19-macos-sitting-c.md`. B06 completed, B08 `verify ok`, and `/mcp` in a real session shows `servicenow · ✔ connected · 397 tools` through the LOCAL entry, agreeing with `E-27` (`Connected · scope local`) and `SV-05` (397 tools matching the contract) |
...
| ARC-07 R9, ARC-07-S01 AC6 | S-04 masked-input matrix (8 cells; `WINDOWS_KNOWN_BAD` ships empty in `packages/snowarch/src/cli/tty.ts`); fallback `--password-stdin` documented | "S-04, the masked-input matrix (ARC-07-S01)" | open |
