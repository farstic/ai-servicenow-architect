...
| 7.4 | No git on PATH | windows-latest branch run with `C:\Program Files\Git\cmd` also stripped | `.\bootstrap.cmd --mode design --yes` exits **3** printing **`git not found`** with remedy `winget install Git.Git` (`bootstrap.ps1:132`). *Corrected per B09-02:* the old expectation `git ≥ 2.25 not found` is a literal the launcher never prints and conflates two paths — absent (:132) vs below floor (:133, `git <v> found, >= 2.34.1 required`). Pinned by `tests/launcher-parity.test.mjs` | ARC-09-S08 AC4 (B09-02) |
