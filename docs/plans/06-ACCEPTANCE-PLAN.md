...
| 3.4 | Design-only validation run | Run T-01 to T-18 from `tests/VALIDATION-TESTS.md` on the RC (CLI version recorded); write `docs/spikes/validation-runs/<date>-design-only.md` (precedent: 18/18 at 31b3c97 on 2.1.258) plus the two-turn T-01 record; run the T-01 structure regression (`scripts/maint/t01-regression.mjs`, seven-element set after B02-03) | 18/18 PASS; T-01 structure equals the contract set | ARC-02 R6, R7, S13 AC5, S08 AC6 |
