...
3. With `SNOW_STORE=/nonexistent` exported in the shell, B08 fails with the "different store" sentence naming `SNOW_STORE`. **AMENDED (ARC-06 acceptance, 2026-09-13, on 06d9f71): superseded by ARC-08-S05's design, and no longer possible.** There is no "different store" sentence in the product; the only one naming the variable is SV-06's remedy in `packages/snowarch/src/doctor/checks.ts`. And the ambient value is neutralised on purpose — `saved` in `tools/snowarch/lib/doctor/checks/server.mjs` saves it, replaces it with `ctx.storePath` and restores it, because *"it makes a doctor run describe the root it was GIVEN, which is what B08, the CI smoke probe and every fixture-based test rely on."* Measured: `./snowarch doctor --json --no-network` with the variable unset and with `SNOW_STORE=/nonexistent/store.json` returns the same status for all ten SV checks. What is asserted instead is the invariant that replaced it — the server reads the store it was GIVEN (`tools/snowarch/tests/server-store-env.test.mjs`) — and SV-06's remedy is now asserted to name the variable (`sv06-store-drift.test.ts`).
4. With `MCP_TIMEOUT` set to `1` in a test copy of `.claude/settings.json`, B08 fails with the `initialize` timeout sentence (S-06 negative control); with the S-06 value it passes. **AMENDED (ARC-06 acceptance, 2026-09-13, on 06d9f71): `MCP_TIMEOUT` does not bound the handshake and never has.** The deadline is fixed in the server package (`checks.ts`, `HANDSHAKE_TIMEOUT_MS` = 20 000 ms); `MCP_TIMEOUT` is the number the cold start is compared *against*, read by the ENGINE from `.claude/settings.json` for a headroom warning — the package says so at `checks.ts:404` and `:319`, and the engine consumes it at `server.mjs:153`. Setting it to `1` produces no timeout. Asserted instead: the timeout sentence against a server that never answers, naming the duration it was given and **not** naming `MCP_TIMEOUT` (`packages/snowarch/tests/doctor/handshake-timeout.test.ts`), and the headroom warning in both directions (`tests/doctor/mcp-timeout-headroom.test.mjs`). The mismatch — a hardcoded 20 s regardless of `MCP_TIMEOUT` — is **ARC-06-C2**, a 2.0.x chore.
5. With `dist/contract.json` edited to drop one tool, B05 fails first; with `required-tools.json` edited to add a fictitious tool and the sha updated, B08 fails naming the tool and its `used_by`. **AMENDED (ARC-06 acceptance, 2026-09-13, on 06d9f71): the second half names the wrong step and a field nothing emits.** It is **B05** that catches a pinned tool the server does not have — `checkContract` in `tools/snowarch/lib/steps/B05.mjs` — and its message is *"the engine pins N tool(s) the server does not have (…names…)"*. **`used_by` is never rendered anywhere**: `grep -rn used_by` over `tools/` and `packages/snowarch/src` matches one test fixture and no product code; it is documentation inside `required-tools.json` for a person reading the file. B08 adopts failures from `SV-01, SV-05, SV-06, SV-04` only and performs no required-tools comparison. Already covered by `tools/snowarch/tests/b03-b05.test.mjs` ("B05 also catches a pinned tool the server does not have"), which asserts that message.
6. `grep -Ei "password|authorization|https://" .local/doctor-last.json .local/logs/*.log` prints nothing after a live run.
7. The handshake module, given a fixture server that answers `tools/list` in three pages, returns all names; given one that never answers, rejects after the timeout without a hung child (`ps` shows none). **AMENDED (ARC-06 acceptance, 2026-09-13, on 06d9f71): the paging clause describes a capability neither side has, and is dropped.** The handshake sends one `tools/list` and reads `result.tools` once (`checks.ts:376/378`); the server returns `{ tools: advertised }` with no cursor (`server.ts:75`, and `grep -c nextCursor packages/snowarch/src/server.ts` = 0). The second clause is real and is now proven: `packages/snowarch/tests/doctor/handshake-timeout.test.ts` rejects after the deadline and asserts no stub survives, **with a control** — a stub told to ignore SIGTERM, which does survive — so the "it is gone" assertion is shown able to fail. A single-page read where a server paged would silently under-count: **ARC-06-C3**, a 2.0.x chore.
