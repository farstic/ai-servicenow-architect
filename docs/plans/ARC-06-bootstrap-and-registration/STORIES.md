...
3. With `SNOW_STORE=/nonexistent` exported in the shell, B08 fails with the "different store" sentence naming `SNOW_STORE`. **AMENDED (ARC-06 acceptance, 2026-09-13, on 06d9f71): superseded by ARC-08-S05's design, and no longer possible.** There is no "different store" sentence in the product; the only one naming the variable is SV-06's remedy at `packages/snowarch/src/doctor/checks.ts:510`. And the ambient value is neutralised on purpose — `tools/snowarch/lib/doctor/checks/server.mjs:80-83` saves it, replaces it with `ctx.storePath` and restores it, because *"it makes a doctor run describe the root it was GIVEN, which is what B08, the CI smoke probe and every fixture-based test rely on."* Measured: `./snowarch doctor --json --no-network` with the variable unset and with `SNOW_STORE=/nonexistent/store.json` returns the same status for all ten SV checks. What is asserted instead is the invariant that replaced it — the server reads the store it was GIVEN (`tools/snowarch/tests/server-store-env.test.mjs`) — and SV-06's remedy is now asserted to name the variable (`sv06-store-drift.test.ts`).
