...
7. **The contexts generator was verified against a live run, not trusted.** Its first output named thirteen `bootstrap` cells that no run produces, because `include:` rows were split on every comma and `label: 'ubuntu-latest, node 20'` carries one. Fixed, and the comparison against the run is what caught it.
