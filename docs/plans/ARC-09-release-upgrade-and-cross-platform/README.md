...
| ID | What | Status |
|---|---|---|
| ARC-09-C4 | `scripts/ci/banner-timing.mjs` judged the raw median of five cold hook spawns against a 1000 ms budget — most of which, on `bootstrap (no-gitbash, windows-latest)`, is Node starting up. The same cell measured 728, 802, 944 and **1027 ms** across four consecutive runs whose product code was byte-identical (`git diff 09243a2..b9bdab3 -- tools/ scripts/ packages/` is empty), and the fourth failed the budget the first three passed | **Closed** — the harness measures an empty-Node floor, INTERLEAVED with the real runs so both see the same weather, and spends the budget on the difference. Both numbers are printed: "the banner cost 190 ms on a machine where starting node costs 840" is the sentence a reader needs. Locally: median 30 ms, floor 23 ms → banner 7 ms |
