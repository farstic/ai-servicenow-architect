...
2. With `STUB_CONFIGURED=1`, `tools/list` returns eight tools including `snow_core_record_add` and `snow_probe_env_read`; a `tools/call snow_probe_env_read` with `HTTPS_PROXY=http://u:p@proxy.invalid:3128` in the environment prints `HTTPS_PROXY: set (len 31)` and never the value.
...
- `LICENSE`: the unmodified Apache License 2.0 text (the SPDX canonical text). No appendix boilerplate edits.
