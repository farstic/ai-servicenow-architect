...
**Scope.** In: the three spikes on macOS (both Claude Code versions) and Windows (`gitbash` snapshot for S-16 — the doctor is invoked through Claude's Bash tool; `clean` snapshot for S-01/S-17). Out: the bootstrap itself (ARC-06) — the spike writes the toggle file by hand exactly as B07 will; the real server (S-17 uses the stub in unconfigured mode, which advertises the same five tools `01` §9 and the ARC-04 README list; `03` S-17 names all five tools — verified 2026-09-06; an earlier draft claimed it said "four").
...
- **S-10** → `Deferred → ARC-04` (after the SCRIPTING read/write split): "run VALIDATION-TESTS T-01…T-18 in `read-only` and `pdi-developer` on a PDI; verdict decides whether Code Review of live scripts needs `pdi-developer`." Owner: **ARC-04-S05** (the gate-split story) runs it and writes `docs/spikes/S-10-readonly-preset-sufficiency/README.md`; ARC-00 cannot — it ends before ARC-01 begins, so ARC-04-S05's tasks "hand the S-10 checklist to ARC-00" are to be read as "run the checklist recorded here". ARC-08 doctor note if it fails.
