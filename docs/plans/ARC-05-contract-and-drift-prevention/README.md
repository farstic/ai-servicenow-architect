...
| B05-06 — S08 AC 7: the contract test under 30 s | **record — measured: 1,295 ms against a 30 s cap** | The row said to read the duration from the `contract (windows-latest, node 20)` cell. **That cell runs `npm run contract` and never invokes vitest**, so the number cannot be there; it is in the `test (windows-latest, node 20)` cell. Corrected in the plan AND in the RC run order §3 row 6.1, which named the same cell. **The measurement, from this PR's own green run** — run `34729868657` on `f91d16f`, job `103650549196` (`test (windows-latest, node 20)`, conclusion success), the line verbatim:<br>`✓ tests/contract.test.ts (444 tests) 1295ms`<br>So the criterion — both flag states for every catalogue tool, under 30 s on the slowest runner — is met with **23× headroom**, and the finding is that it was never close. It took two attempts to get an honest number: the first run on this branch was RED (see the row above), and a duration read out of a failed run is not a measurement |
