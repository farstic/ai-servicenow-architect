...
| B01-07 — S12 **AC 2**: a Product constants row per config leaf key | **rework** | **The first pass read AC 1 and stopped, and AC 2 is a real gap rather than a stale criterion.** Measured: `docs/ARCHITECTURE.md` had no such table, and `docs.upstream`, `roster.utility` and `mcp.permissions.*` appeared nowhere under `docs/` at all — the schema beside the file is strict but carries 13 descriptions for 18 keys, so it did not document them either. Written: a *Product constants* section, one row per leaf key, each naming what it is and **who reads it**, with no values repeated — the file is the source, and a value copied into prose is a second source that drifts. `tests/version-consistency.test.mjs` asserts both directions (a key with no row, a row with no key), no duplicate row, and that the pin is not repeated in the table |
...
| ID | What | Status |
|---|---|---|
| ARC-01-C1 | B01-02's criterion is a CONJUNCTION and the first pass asserted one half — the grep, not the clause that ADR-0002 must still quote the superseded licence | **Closed** — asserted, together with the fact that ties the two: the ADR is exempt BECAUSE it quotes it, so an exempt file that stopped quoting is a stale exemption, the same shape this file already guards for the other carriers |
| ARC-01-C2 | B01-07 AC 2 unaddressed and unmentioned — see the acceptance row above | **Closed** — the table and its two-direction test |
| ARC-01-C3 | B01-04 narrowed the proposed check — the workspace VERSIONS were dropped and the row did not say so. **Silent narrowing is the finding, not the two lines** | **Closed** — `lock.packages['packages/snowarch'].version` and `['tools/snowarch'].version` are asserted equal to the root version; `npm version` keeps them in step and a hand edit of one `package.json` is the drift nobody reads |
| ARC-01-C4 | The force-push proof was missing: a subject-only amend on an unmerged branch needs before/after shas and an empty diff between them, and the PR body had neither. The branch was pushed before the PR existed, so no `head_ref_force_pushed` event can recover the before sha | **Closed** — both shas are in the PR body: before `b07264b`, after `89bb06f`, `git diff --stat b07264b 89bb06f` empty |
| ARC-01-C5 | `FORBIDDEN_PATHS` had no positive control, and this PR added three regexes to it. A pattern that can match nothing — one typo away — passes for ever, because nothing tracked matches it either | **Closed** — the thirteen cut paths are spelled out as literals; every literal matches a pattern, every pattern matches a literal, and two live paths must NOT match, so a pattern that widened into the tree is caught too |
| ARC-01-C6 | `bootstrap (no-node, macos-latest)` went red on `89bb06f` with an HTTP 408. A red cell on a PR that touched none of that code is exactly the shape ARC-09-C30 was written about: **a re-run without a cause is what the standing rule forbids** | **Closed, and it was not a flake.** The 408 was real and transient, but what the cell PROVED is a product defect: the Node-free recipe swallowed the failure whole and the doctor reported the symptom three steps later. Cause, fix and controls are ARC-03-C1, where the code lives. No re-run was used to make this cell green |
| ARC-01-C7 | **Twelve of the plan set's fifty-one live code citations had already drifted, and every one still read as precise.** Measured on 2026-09-19 while sweeping `file:line` citations to expressions (the convention in `docs/CONTRIBUTING.md`, *Citing code in a plan row*). A drifted citation is not a broken link — nothing fails, nothing warns, and the row around it still reads as though somebody checked. That is the whole hazard: **the reader who follows one lands on unrelated code and assumes they misread the row.** | **The twelve, with what each actually landed on:** `.claude/agents/now-assist-specialist.md:71` — a bullet about attestation templates; the step it cites is at `:74` (counted twice, cited from `03-RISKS-AND-UNKNOWNS.md` and `ARC-03…/STORIES.md`) · `bootstrap.sh:200` — `B02_S=$(( … ))`, a timing variable · `tests/doctor/host.test.mjs:83` — a bare `});`, the named skip is at `:86` · `docs/CONTRIBUTING.md:1923` — an unrelated paragraph about a v1 store migration; the table it names is at `:1979` · `docs/spikes/OWNER-SITTING.md:150` — a blank line; the heading it defines is at `:203` · `package.json:54` — **past the end of the file** · `packages/snowarch/README.md:83` — a blank line · `docs/ARCHITECTURE.md:133` — a sentence about proxy and CA variables; the D-03 ledger is at `:1447` · `docs/INSTALL.md:49` — a bare code-fence line; the sentence quoted is at `:58` · `tools/snowarch/lib/docs/sync.mjs:292` — `const probe`, not the `ROOT_FILES` it names · `tools/snowarch/lib/docs/sync.mjs:247` — `attemptSuffix`, not the `plan === null` it names. **One more is named PARTIAL rather than counted:** `packages/snowarch/src/doctor/checks.ts:510` lands on SV-06's `title:` line, inside the right check, two lines above the remedy the row cites — a reader finds it. The twelve is a floor whose edge is visible, not a number to trust blindly. **Two of the twelve were found only on a recount.** `sync.mjs:247` and `sync.mjs:292` were swept correctly — each target line was read before it was rewritten — but were missed when the total was first quoted as "roughly a third". The sweep was right and the arithmetic about it was not, which is the same defect one level up: **a count reads as precise exactly the way a citation does.** "Roughly a third" became "17" in a reviewer's hands before it was caught, one message short of a permanent rule. **Method, stated so it is not silently redone: a hand reading of all 51.** Automation was tried and its result discarded — a token-overlap check between each claim and its cited line reported **31** drifted, flagging `bootstrap.ps1:132` (which lands exactly on `Die 'B00' 'git not found'`) and `packages/contract/gen/protocols.mjs:18` (which lands exactly on `ATF_ENABLED`), because quoting and unicode differ between a row and the code it quotes. A heuristic that cannot tell a hit from a miss is not a measurement. The next person's instinct will be to automate this; the automation is available, fast, and wrong in the direction that looks like diligence. **This row's own `file:line` tokens are the exception that proves the rule and must not be swept:** they are not pointers, they are the QUOTED EVIDENCE — the rotted citations themselves, reproduced so the drift can be seen. Rewriting them to expressions would delete the finding while appearing to enforce it, which is the one way this row can be destroyed by somebody following the very convention it justifies |
