...
**Acceptance criteria.**
1. `git grep -n -i -E "source available|all rights reserved|not licensed for redistribution|license: MIT|SEE LICENSE IN LICENSE" -- . ':!LICENSE' ':!packages/snowarch/LICENSE' ':!NOTICE' ':!docs/plans' ':!docs/decisions' ':!docs/spikes'` prints nothing — **and** `git grep -ci -E "source.available" docs/decisions/ADR-0002-licence.md` is ≥ 1 (measured: **2**), so the evidence is still there.
