...
2. `git log --oneline -- CLAUDE.md | wc -l` ≥ 20 and `git log --format=%H -- CLAUDE.md | tail -1` equals the engine's first commit touching `CLAUDE.md` (compare with `git -C ~/work/AI-Architect-Claude log --format=%H -- CLAUDE.md | tail -1`).
3. `diff -rq --exclude=.git --exclude=ServiceNowDocs --exclude=node-compile-cache --exclude=scratchpad --exclude=.backups --exclude=.DS_Store --exclude=settings.json --exclude=.mcp.json --exclude=SETUP.md --exclude=setup.sh --exclude=doctor.sh --exclude=.gitmodules --exclude=LICENSE --exclude=NOTICE --exclude=ARCHITECTURE.md --exclude=legacy ~/work/AI-Architect-Claude/ ./` prints nothing (byte-identical import of everything not on the exclusion/move list) — under relicensing resolution (b) the files on the `RELICENSING.md` rewrite list are added to the `--exclude` set and are checked instead by criterion 9.
...
7. `git grep -l -E '"(password|client_secret|clientSecret)"\s*:\s*"' -- . ':!packages' ':!*.example.json'` prints nothing (no secret from the gitignored engine files crossed over).
...
2. `git log --follow --oneline packages/snowarch/src/server.ts | tail -1` is the same commit as `git -C ~/work/snow-mcp log --oneline -- src/server.ts | tail -1` (history reachable through the subtree merge).
...
- The engine's `docs/CHANGELOG.md` (stops at 2.7.6) is imported as-is; ARC-09 regenerates it. The test only guards ordering, so the stale file does not fail ARC-01.
...
- Ratchet patterns added to S10's forbidden list (no allow-list entries permitted): `Source Available License`, `All rights reserved. Internal use only`, `not licensed for redistribution`, `license: MIT`, `SEE LICENSE IN LICENSE`.

**Acceptance criteria.**
1. `git grep -n -i -E "source available|all rights reserved|not licensed for redistribution|license: MIT|SEE LICENSE IN LICENSE" -- . ':!LICENSE' ':!packages/snowarch/LICENSE' ':!NOTICE'` prints nothing.
