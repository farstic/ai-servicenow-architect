...
1. `git grep -n -i -E "source available|all rights reserved|not licensed for redistribution|license: MIT|SEE LICENSE IN LICENSE" -- . ':!LICENSE' ':!packages/snowarch/LICENSE' ':!NOTICE' ':!docs/plans' ':!docs/decisions' ':!docs/spikes' ':!docs/ARCHITECTURE.md' ':!tests/no-legacy-names.test.mjs' ':!.gitleaksignore'` prints nothing — **and** `git grep -ci -E "source.available" docs/decisions/ADR-0002-licence.md` is ≥ 1 (measured: **2**), so the evidence is still there.
   ...
   ***Correction 2026-09-08 (folded into S12).** Clause (a) acquired two matches after S08 was written, and **neither is a licence claim** — they are the two classes the ARC-01-S10 ratchet already exempts, for the same reasons, which is why the clause now uses the same exemption set rather than a pathspec that grows: **detectors**, code whose job is to match the old string (`tests/no-legacy-names.test.mjs` is the ratchet's own pattern list; `.gitleaksignore` names the rules it suppresses), and the **historical record** (`docs/ARCHITECTURE.md`'s *D-03 cut ledger*, the D-03 cut ledger, which quotes *"Source Available License — All rights reserved"* to say what was removed and why). The architect's exit-checklist run found the first; the second appeared with S12's own ledger. Measured after the exclusion: **0 files**.)*
