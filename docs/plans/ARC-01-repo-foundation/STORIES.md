...
- Ratchet patterns added to S10's forbidden list (no allow-list entries permitted): `Source Available License`, `All rights reserved. Internal use only`, `not licensed for redistribution`, `license: MIT`, `SEE LICENSE IN LICENSE`. *(Amended 2026-09-08: these five must be scanned with `':!docs/plans' ':!docs/decisions' ':!docs/spikes'`, for the reason in criterion 1 — the decision record and the spike archive quote the superseded texts deliberately, and 11 of the 12 pre-sweep hits were exactly that. A ratchet without those exclusions would force 11 allow-list rows on day one, or the deletion of ADR-0002's evidence. **Write them case-insensitively with a tolerant separator** (`source.available`, not the literal `Source Available`): the decision record spells it "source-available" and "Source-available", and neither matches the literal form — S08b was exactly that mistake, caught in review.)*

**Acceptance criteria.**
1. `git grep -n -i -E "source available|all rights reserved|not licensed for redistribution|license: MIT|SEE LICENSE IN LICENSE" -- . ':!LICENSE' ':!packages/snowarch/LICENSE' ':!NOTICE' ':!docs/plans' ':!docs/decisions' ':!docs/spikes' ':!docs/ARCHITECTURE.md' ':!tests/no-legacy-names.test.mjs' ':!.gitleaksignore'` prints nothing — **and** `git grep -ci -E "source.available" docs/decisions/ADR-0002-licence.md` is ≥ 1 (measured: **2**), so the evidence is still there.

   ***Correction 2026-09-08 (folded into S12).** Clause (a) acquired two matches after S08 was written, and **neither is a licence claim** — they are the two classes the ARC-01-S10 ratchet already exempts, for the same reasons, which is why the clause now uses the same exemption set rather than a pathspec that grows: **detectors**, code whose job is to match the old string (`tests/no-legacy-names.test.mjs:20,27` is the ratchet's own pattern list; `.gitleaksignore` names the rules it suppresses), and the **historical record** (`docs/ARCHITECTURE.md:133`, the D-03 cut ledger, which quotes *"Source Available License — All rights reserved"* to say what was removed and why). The architect's exit-checklist run found the first; the second appeared with S12's own ledger. Measured after the exclusion: **0 files**.)*
   ...
   ***Correction 2026-09-08 (S08b), and it is a correction to this amendment rather than to the sweep.** The positive clause was first written as `git grep -c "Source Available" …` and reported as passing **without being run**: it returns **0**, because ADR-0002 writes the term **hyphenated and in lower case** — "source-available" at line 21 and "Source-available" at line 47. The clause is now `git grep -ci -E "source.available"`, which is case-insensitive and tolerates the separator, and returns **2** on the committed tree. **The same case/separator tolerance is required of S10's ratchet exclusions**: a pattern written as the literal `Source Available` matches neither spelling that actually appears in the record, so a ratchet built on it would silently protect nothing.)*
...
6. `git grep -h -o -E "[a-z0-9-]+\.service-now\.com" -- . ':!scripts/legacy' ':!docs/ARCHITECTURE.md' | sort -u` prints at most the documentation placeholder `dev12345.service-now.com` (the host `01` §6.2 and the server docs use as the example), and none of the hostname fragments recorded in the maintainer's local `memory/MEMORY.md` (never committed — the fragments are checked by hand from that file, not spelled in this plan). Measured 2026-09-04: the engine's tracked files contain no `service-now.com` host at all; the untracked `scripts/setup.sh` (moved to `scripts/legacy/` by S02) contains only `dev12345.service-now.com`. *(Amended 2026-09-08. That measurement predates ARC-01-S03's server import, which brought the server's documentation with it: the tree now holds **26 distinct hosts**, so "at most `dev12345`" cannot pass and never could after S03. Every one is visibly a placeholder — `acme`, `yourcompany`, `dev`, `prod`, `dummy`, `myinstance`, `globex`, `x`, `y`, `test`, `customera` and so on — and the criterion's **substantive** half is the one that matters and is now automated rather than done "by hand": the set of hosts in the tree is intersected with the real fragments read from the maintainer's local, never-committed `memory/` and global `CLAUDE.md`. **Measured: 26 hosts in the tree, 1 real fragment known locally, intersection 0** — and no value is printed by the check. The criterion is therefore "no host in the tree matches a real instance", not "only one placeholder exists".)* `scripts/legacy/` is reference material and is checked by hand once for hostnames — if any real one appears, redact in the import commit of S02.
