...
| Keyword/phrase | Primary specialist | Secondary specialist |
|---|---|---|
| "incident", "problem", "change", "RITM", "SLA", "escalation", "Service Operations Workspace", "MIM", "on-call" | ITSM Specialist | — |
...
> **These rows are triggers, not definitions.** A task can be squarely in a domain and match no row — the gateway fires on the domain (§6.1 step 7). The ITSM row also carried less than CLAUDE.md's copy of the same list until 2026-09-17 (`RITM` and `SLA` were missing, `escalation` was in neither): two hand-kept copies of one list drift, which is the second reason the list cannot be the definition.
...
7. **Apply the Domain Expert gateway (mandatory — this is CLAUDE.md Phase 1 Step 5).** Check whether the task falls within a domain covered by a v2.0 gateway (ITSM, CSM, HRSD, ITOM, CMDB & CSDM — see §4.4 domain triggers). If yes, load and adopt the relevant Domain Expert skill before dispatching any builder — **or before finalizing a domain-scoped *document* deliverable (implementation proposal, scoping document, HLD/LLD/PDD) that asserts baseline-process, data-model, or §1.1 claims about a gateway domain.** The Domain Expert produces its 5-Part Constraint Envelope (OOB Process Map · Data Model Alignment · §1.1 Verdict · Routing Recommendation · Anti-Patterns). **For document deliverables** the gateway grounds/ratifies the document's domain claims even with no builder dispatched; for a multi-domain document each touched gateway fires and sections are marked *gateway-ratified* vs *freehand, pending gateway*. **Multiple gateways co-fire** for cross-domain tasks (e.g., CSM ↔ ITSM ↔ CSDM) — reconcile their envelopes into one dispatch context. For the CMDB & CSDM ↔ ITOM/Discovery boundary, ITOM owns CI *population* and CMDB & CSDM owns the CI *model*; fire both only when the task spans both. No builder sub-agent is dispatched until every Envelope is produced and the §1.1 Verdict is resolved. Verdict C (custom object required) is a hard stop — surface the OPEN QUESTION and wait for explicit user approval before proceeding. **Part 3 is the turn's conclusion: no closing recommendation, proposed default or summary may lean against it or against Part 5, and an Architect who disagrees with the gateway raises that disagreement as its own OPEN QUESTION rather than as a default.** **Adoption is an action in the turn, with a trace: invoke the gateway's skill (`Skill` with its name) or read `.claude/skills/<gateway>/SKILL.md`, then write the Envelope.** A gateway is a skill, never a sub-agent — nothing to dispatch, propose or approve; a turn that asks permission to run one, declares a Verdict with no Envelope, or loads nothing has not fired it, and the Envelope does not wait on an engagement folder. Naming a custom object fixes Part 3 to Verdict C and nothing else — Parts 1, 2, 4 and 5 are still written. **§4.4 is a trigger, not a definition, and the gateway is never deferred.** It fires on the DOMAIN — a scope fragment, a baseline table, or the domain's own semantics fire it with no listed keyword present — and *"it fires once X is known"* is a gateway that did not fire; unknowns are recorded IN the Envelope. When it is unclear whether a domain applies, fire. (Observed: T-10, 2026-09-12 — the prompt named `x_acme_itsm_escalation_log` and escalation semantics, matched no §4.4 ITSM keyword, and the gateway was made conditional on a parent table, so the §1.1 halt was raised generically instead of as Part 3.)
