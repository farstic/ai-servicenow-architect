...
const warn = (id: Check['id'], title: string, detail: string, remedy?: string): CheckResult =>
  ({ id, title, status: 'warn', detail, ...(remedy ? { remedy } : {}), fixable: false });
const fail = (id: Check['id'], title: string, detail: string, remedy?: string): CheckResult =>
  ({ id, title, status: 'fail', detail, ...(remedy ? { remedy } : {}), fixable: false });
const skip = (id: Check['id'], title: string, detail: string, remedy?: string): CheckResult =>
  ({ id, title, status: 'skip', detail, ...(remedy ? { remedy } : {}), fixable: false });

// ─── SV-00 — Node floor ──────────────────────────────────────────────────────

export const svNodeFloor: Check = {
  id: 'SV-00',
  title: 'Node version',
  severity: 'fail',
  network: false,
  async run() {
    ...
    return major >= 20
      ...
      : fail('SV-00', 'Node version', `node ${process.versions.node} is below the floor of 20`,
...
export const svDist: Check = {
  ...
  async run() {
    ...
    if (missing.length > 0) {
      return fail('SV-01', 'dist artefacts',
        `missing: ${missing.map((p) => maskPath(p)).join(', ')}`,
        'run npm run build in packages/snowarch');
    }
    try {
      const text = readFileSync(contract, 'utf8');
      const parsed = JSON.parse(text) as { toolCount?: number };
      const sha = createHash('sha256').update(text).digest('hex');
      return ok('SV-01', 'dist artefacts',
        `server.js present, contract ${parsed.toolCount} tools, sha256 ${sha.slice(0, 12)}`);
    } catch (e) {
      return fail('SV-01', 'dist artefacts', `contract.json is not readable JSON: ${(e as Error).message}`,
        'run npm run build in packages/snowarch');
    }
  },
};
...
export const svStore: Check = {
  ...
  async run() {
    ...
    if (res.path === null) {
      ...
      return warn('SV-02', 'store', 'no store found — the server starts unconfigured',
...
export const svInstances: Check = {
  id: 'SV-03',
  title: 'instances',
  severity: 'fail',
  network: false,
  async run(ctx: CheckContext) {
    ...
    for (const i of loaded) {
      ...
      for (const w of entry.warnings) notes.push(`${i.name}: ${w}`);
...
// ─── SV-05 / SV-06 — a real stdio handshake against ourselves ────────────────
    ...
    child.stdout.on('data', (chunk: Buffer) => {
      ...
      while (nl !== -1) {
        ...
        if (msg.id === 1) {
          ...
          const tools = ((msg.result as { tools?: Array<{ name: string }> })?.tools ?? []).map((t) => t.name);
          ...
          try { capabilities = text ? JSON.parse(text) as Record<string, unknown> : null; } catch { capabilities = null; }
          finish({ tools: (seen[2] as string[]) ?? [], capabilities });
        }
      }
    });
...
export const svHandshake: Check = {
  ...
  async run() {
    ...
    if (h.error) {
      return fail('SV-05', 'stdio handshake', `the server did not answer: ${h.error}`,
        'run node packages/snowarch/dist/server.js and read its stderr');
    }

    const declared = (JSON.parse(readFileSync(contractPath, 'utf8')) as { tools: Array<{ name: string }> })
      .tools.map((t) => t.name);
    ...
    return fail('SV-05', 'stdio handshake', parts.join('; '),
      'the contract and the build disagree — run npm run build in packages/snowarch');
  },
};

export const svCapabilities: Check = {
  id: 'SV-06',
  ...
  async run() {
    ...
    if (!entry) return skip('SV-06', 'capabilities match the store', 'no current instance');

    const c = h.capabilities as {
      instance?: string; preset?: string; environment?: string; maxRecords?: number;
      flags?: Record<string, string>; effectiveFlags?: Record<string, string>;
    };

    const differences: string[] = [];
    const compare = (field: string, live: unknown, stored: unknown): void => {
    ...
    if (c.flags) compare('flags', c.flags, entry.flags);
    if (c.effectiveFlags) compare('effectiveFlags', c.effectiveFlags, entry.effectiveFlags);

    return differences.length === 0
      ? ok('SV-06', 'capabilities match the store',
        `${entry.label}: preset ${entry.preset}, ${entry.environment}, maxRecords ${entry.maxRecords}`)
      : fail('SV-06', 'capabilities match the store', differences.join('; '),
        'the running server is not reading the store this doctor read — restart it, and check SNOW_STORE');
  },
};

// ─── SV-07 — the audit file is writable ──────────────────────────────────────

export const svAudit: Check = {
  id: 'SV-07',
  title: 'audit trail',
  severity: 'warn',
  network: false,
  async run() {
    ...
    if (p === null) {
      return warn('SV-07', 'audit trail', 'disabled by SNOW_AUDIT_FILE=off — no §2.1 evidence is being kept',
        'unset SNOW_AUDIT_FILE to record mutating calls again');
    }
    ...
    try {
      ...
      return warn('SV-07', 'audit trail', `${maskPath(p)} is not writable — mutating calls will not be recorded`,
        `make ${maskPath(dir)} writable, or set SNOW_AUDIT_FILE to a path you own`);
    }
  },
};
 ...
 */
export function pollutingAncestors(dir: string, home: string = homedir()): string[] {
  const found: string[] = [];
...
export const svAncestorSkills: Check = {
  id: 'SV-08',
  ...
  async run(ctx: CheckContext) {
    ...
    return ancestors.length === 0
      ...
      : warn('SV-08', 'ancestor skill directories',
        `skills are also loaded from: ${ancestors.map((p) => maskPath(p)).join(', ')} — `
        + 'the roster is larger than this repository defines and the listing budget is spent twice',
        'disable their skills with /skills, or move the checkout out from under them');
  },
};
 ...
 */
export const svStoreSchema: Check = {
  id: 'SV-09',
  title: 'store schema',
  severity: 'fail',
  network: false,
  async run() {
    ...
    let version: unknown;
    try {
      version = (JSON.parse(readFileSync(res.path, 'utf8')) as { version?: unknown }).version;
    } catch {
