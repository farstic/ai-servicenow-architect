...
export function statusOf(result: ProbeAll): CheckStatus {
  if (result.auth.status === 'auth failed') return 'fail';
  if (result.auth.status !== 'ok') return 'warn';
...
export function makeProbes(deps: ProbeBindingDeps = {}): Probes {
