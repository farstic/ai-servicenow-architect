...
export const storeSubCommandList = (): string => {
 ...
 */
export function subCommandLines(
  entries: ReadonlyArray<{ usage: string; summary: string }>,
  gap = 2,
): string[] {
...
export const instanceSubCommandLines = (): string[] => subCommandLines([
  INSTANCE_WIZARD_LINE,
  ...Object.entries(SUB_COMMANDS).map(([usage, meta]) => ({ usage, summary: meta.summary })),
]);

/** The `store` sub-command lines. */
export const storeSubCommandLines = (): string[] =>
  subCommandLines(Object.values(STORE_SUB_COMMANDS), 3);
