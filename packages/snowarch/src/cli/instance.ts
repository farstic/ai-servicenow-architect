 ...
 */
export const CLOUD_SYNC_CHOICES: ReadonlyArray<Option> = Object.freeze([
...
export const storeLine = (path: string, platform: NodeJS.Platform = process.platform,
  project: string = projectStorePath()): string => {
...
export function stepHeader(id: StepId, suffix = ''): string {
  ...
  if (at < 0) throw new Error(`no wizard step "${id}" — the ids are ${STEPS.map((s) => s.id).join(', ')}`);
...
export async function cloudSyncGate(storePath: string, options: { yes?: boolean; global?: boolean },
  ...
  const chosen = await askOption(io, `This folder is inside ${hit.provider} — the saved password `
    + 'file would sync with it.', CLOUD_SYNC_CHOICES,
  { defaultKey: 'stop',
