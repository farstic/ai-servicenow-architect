...
import { statusWords, type LastProbe, type ProbeStatus } from '../servicenow/probes.js';
...
function annotationParts(status: ProbeStatus | undefined, hint?: string,
  row?: number): { head: string; tail: string; command?: string } {
...
export const annotationCommand = (status: ProbeStatus | undefined): string | null =>
  annotationParts(status).command ?? null;
...
export function renderReviewScreen(input: ScreenInput): string {
  ...
  lines.push(...wrapRow('',
    `Permissions for instance "${label}" (environment ${environment}) — what Claude's tools may do `
    + 'there.'));
  ...
  FLAG_NAMES.forEach((flag, i) => {
    ...
    if (command) lines.push(`${' '.repeat(prefix.length)}${command}`);
  });
