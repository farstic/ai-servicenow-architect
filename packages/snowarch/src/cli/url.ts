...
export const ENVIRONMENTS = ['pdi', 'dev', 'test', 'prod'] as const;
export type Environment = (typeof ENVIRONMENTS)[number];

const bad = (code: ErrorCodeName, message: string): UrlBad => ({ ok: false, code, message });
...
export function normalizeInstanceUrl(input: string): UrlResult {
  ...
  if (/^http:\/\//i.test(trimmed)) {
    return bad('URL_NOT_HTTPS',
      'ServiceNow instances are served over https only; use https://<host>');
  }
  ...
  if (parsed.username !== '' || parsed.password !== '') {
    ...
    return bad('URL_HAS_CREDENTIALS',
  ...
  if (parsed.pathname === '/api' || parsed.pathname.startsWith('/api/')) {
    ...
    return bad('URL_HAS_PATH',
      '"/api" is the REST base the server adds itself — enter the bare origin (https://<host>)');
  }
  if ((parsed.pathname !== '/' && parsed.pathname !== '') || parsed.search !== '' || parsed.hash !== '') {
    return bad('URL_HAS_PATH',
      'Enter the instance origin only (https://<host>), without a path');
  }

  const notes: string[] = [];
  if (trimmed.endsWith('/') && parsed.pathname === '/') notes.push('trailing slash removed');
...
export function proposeEnvironment(url: string): Environment | null {
...
export const codeMeaning = (code: ErrorCodeName): string =>
  ERROR_CODES.find((e) => e.code === code)?.meaning ?? code;
...
export async function resolveEnvironment({ url, env, yes, ask }: {
  ...
  if (env !== undefined) {
    ...
    return { ok: true, environment: value as Environment };
  }

  const proposed = proposeEnvironment(url);
  if (proposed) return { ok: true, environment: proposed };
  ...
  return { ok: true, environment: answer as Environment };
}
