...
import { STORE_VERSION, type Store, type StoreError } from '../schema.js';
...
export type VersionedStore = { version: number } & Record<string, unknown>;
 ...
 */
export function checkRegistry(migrations: readonly Migration[] = MIGRATIONS,
  current = CURRENT_SCHEMA_VERSION): string[] {
  const problems: string[] = [];
  if (migrations.length === 0) {
    if (current !== 1) problems.push(`no migrations, but CURRENT_SCHEMA_VERSION is ${current}`);
    return problems;
  }
  for (const [i, m] of migrations.entries()) {
    if (m.to !== m.from + 1) problems.push(`migration ${i} is ${m.from}→${m.to}, not a single step`);
    if (i > 0 && m.from !== migrations[i - 1]!.to) {
      problems.push(`migration ${i} starts at ${m.from}, but ${i - 1} ended at ${migrations[i - 1]!.to}`);
    }
    if (!m.describe.trim()) problems.push(`migration ${i} (${m.from}→${m.to}) has no description`);
  }
  const last = migrations[migrations.length - 1]!.to;
  if (last !== current) problems.push(`the chain ends at ${last}, but CURRENT_SCHEMA_VERSION is ${current}`);
  return problems;
}
 ...
 */
export function authUnchanged(before: VersionedStore, after: VersionedStore): string[] {
  const problems: string[] = [];
  const instancesOf = (s: VersionedStore) =>
    (s.instances ?? {}) as Record<string, { auth?: unknown } | undefined>;
  const a = instancesOf(before);
  const b = instancesOf(after);
  for (const label of Object.keys(a)) {
    if (!(label in b)) { problems.push(`instance "${label}" was removed by a migration`); continue; }
    ...
    if (one !== two) problems.push(`instance "${label}": the auth subtree changed (${one.length} → ${two.length} bytes)`);
  }
  return problems;
}
...
export function migrateStore(path: string, {
  ...
  if (typeof from !== 'number' || !Number.isInteger(from) || from < 1) {
    throw new StoreMigrationError('STORE_SCHEMA_INVALID',
      `${shown}: version must be a positive integer, not ${JSON.stringify(from)}`);
  }
  ...
  if (problems.length > 0) {
    throw new StoreMigrationError('STORE_SCHEMA_INVALID',
      `the migration registry is not usable: ${problems.join('; ')}`);
  }
  if (pending.length === 0 || pending[0]!.from !== from) {
    throw new StoreMigrationError('STORE_SCHEMA_OUTDATED',
      `${shown} is schema ${from}, and this build has no migration from it to ${current}`);
  }

  const steps = pending.map((m) => `${m.from}→${m.to} ${m.describe}`);
  if (dryRun) return { migrated: false, from, to: current, steps, dryRun: true };
  ...
  if (backup) {
    ...
    if (!isWindows) chmodSync(backupPath, 0o600);
  }

  const before = deepFreeze(JSON.parse(text) as VersionedStore);
  let state: VersionedStore = before;
  ...
  if (changed.length > 0) {
    ...
    throw new StoreMigrationError('STORE_SCHEMA_INVALID',
      `a migration changed credential data and was refused: ${changed.join('; ')}`);
  }

  saveStore(path, state as unknown as Store);
  return { migrated: true, from, to: current, steps, ...(backupPath ? { backup: backupPath } : {}) };
}

/** `instances.json.bak-*` beside the store, newest first. */
export function listBackups(storePath: string): { path: string; size: number; at: Date }[] {
 ...
 */
export function restoreBackup(storePath: string, backupPath: string): { version: number } {
  if (!existsSync(backupPath)) {
    throw new StoreMigrationError('STORE_NOT_FOUND', `backup not found: ${maskPath(backupPath)}`);
  }
  let raw: VersionedStore;
  try {
    raw = JSON.parse(readFileSync(backupPath, 'utf8')) as VersionedStore;
  } catch {
    throw new StoreMigrationError('STORE_UNREADABLE',
      `${maskPath(backupPath)} is not valid JSON — it cannot be restored`);
  }
  saveStore(storePath, raw as unknown as Store);
