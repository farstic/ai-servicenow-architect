 ...
 */
export function resolveAuditPath(): string | null {
...
export const auditDisabled = (): boolean =>
  (process.env.SNOW_AUDIT_FILE ?? '').toLowerCase() === 'off';
...
export function resetAuditWarning(): void { warned = false; }
...
export function appendAudit(entry: AuditEntry): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true;
      logger.warn(`audit trail unavailable: ${(e as Error).message}`, { file: maskPath(file) });
    }
  }
}

/** The last `n` lines, parsed. Unparseable lines are skipped rather than throwing. */
export function tailAudit(n = 50): AuditEntry[] {
  const file = resolveAuditPath();
