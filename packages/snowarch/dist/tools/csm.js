...
export async function dispatchCsmAction(client, name, args) {
    switch (name) {
        case 'snow_csm_csm_case_add': {
            requireWrite();
        ...
        case 'snow_csm_csm_case_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`CSM case not found: ${args.number_or_sysid}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_csm_csm_case_modify': {
            requireWrite();
        ...
        case 'snow_csm_csm_cases_index': {
            const parts = [];
            if (args.account)
                parts.push(`account.name=${args.account}`);
            if (args.contact)
                parts.push(`contact.name=${args.contact}`);
            if (args.state)
        ...
        case 'snow_csm_csm_case_close': {
            requireWrite();
            if (!args.sys_id || !args.resolution_notes)
                throw new ServiceNowError('sys_id and resolution_notes are required', 'INVALID_REQUEST');
        ...
        case 'snow_csm_csm_account_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Account not found: ${args.name_or_sysid}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_csm_csm_accounts_index': {
        ...
        case 'snow_csm_csm_contact_read': {
            if (!args.name_or_sysid)
                throw new ServiceNowError('name_or_sysid is required', 'INVALID_REQUEST');
            if (/^[0-9a-f]{32}$/i.test(args.name_or_sysid)) {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Contact not found: ${args.name_or_sysid}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_csm_csm_contacts_index': {
            const parts = [];
            if (args.account_sysid)
                parts.push(`account=${args.account_sysid}`);
            if (args.query)
                parts.push(`nameCONTAINS${args.query}^ORemail=${args.query}`);
            return await client.queryRecords({ table: 'customer_contact', query: parts.join('^') || '', limit: args.limit ?? 25 });
        }
        case 'snow_csm_csm_case_sla_read': {
            if (!args.case_sysid)
                throw new ServiceNowError('case_sysid is required', 'INVALID_REQUEST');
            return await client.queryRecords({ table: 'task_sla', query: `task=${args.case_sysid}`, limit: 10 });
        }
        case 'snow_csm_csm_products_index': {
            const q = args.query ? `nameCONTAINS${args.query}` : '';
