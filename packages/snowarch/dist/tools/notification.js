...
export function notificationToolManifest() {
    return [
        ...
        },
        {
            ...
            description: 'Upload a base64-encoded attachment to a ServiceNow record (requires WRITE_ENABLED=true). ' +
...
export async function dispatchNotificationAction(client, name, args) {
    switch (name) {
        // ── Email Notifications ──────────────────────────────────────────────────
        case 'snow_ntf_notifications_index': {
            const parts = [];
            ...
            if (args.event)
                parts.push(`event_name=${args.event}`);
            if (args.query)
        ...
        case 'snow_ntf_notification_read': {
            ...
            if (resp.count === 0)
                throw new ServiceNowError(`Notification not found: ${args.sys_id_or_name}`, 'NOT_FOUND');
            return resp.records[0];
        }
        case 'snow_ntf_notification_add': {
            requireWrite();
            if (!args.name || !args.table)
                throw new ServiceNowError('name and table are required', 'INVALID_REQUEST');
            const data = {
                name: args.name,
            ...
            if (args.event)
                data.event_name = args.event;
            if (args.subject)
                data.subject = args.subject;
            if (args.message_html)
                data.message_html = args.message_html;
            if (args.recipients)
                data.recipient_fields = args.recipients;
            if (args.condition)
        ...
        case 'snow_ntf_notification_modify': {
            requireWrite();
        ...
        // ── Email Log ────────────────────────────────────────────────────────────
        case 'snow_ntf_email_logs_index': {
            const parts = [];
            if (args.state)
                parts.push(`state=${args.state}`);
            if (args.recipient)
                parts.push(`receiverCONTAINS${args.recipient}`);
            if (args.subject)
                parts.push(`subjectCONTAINS${args.subject}`);
            return await client.queryRecords({
                table: 'sys_email',
                query: parts.join('^') || undefined,
                limit: args.limit ?? 25,
        ...
        case 'snow_ntf_attachment_metadata_read': {
            if (!args.attachment_sys_id)
                throw new ServiceNowError('attachment_sys_id is required', 'INVALID_REQUEST');
            return await client.getRecord('sys_attachment', args.attachment_sys_id);
        }
        case 'snow_ntf_attachment_remove': {
            requireWrite();
            if (!args.attachment_sys_id)
                throw new ServiceNowError('attachment_sys_id is required', 'INVALID_REQUEST');
        ...
        case 'snow_ntf_attachment_upload': {
            requireWrite();
            if (!args.table || !args.record_sys_id || !args.file_name || !args.content_type || !args.content_base64) {
                throw new ServiceNowError('table, record_sys_id, file_name, content_type, and content_base64 are required', 'INVALID_REQUEST');
            }
            const result = await client.uploadAttachment(args.table, args.record_sys_id, args.file_name, args.content_type, args.content_base64);
            return {
                ...result,
        ...
        // ── Templates ────────────────────────────────────────────────────────────
        case 'snow_ntf_email_templates_index': {
            const parts = [];
            if (args.query)
                parts.push(`nameCONTAINS${args.query}`);
            return await client.queryRecords({
        ...
        // ── Subscriptions ────────────────────────────────────────────────────────
        case 'snow_ntf_notification_subscriptions_index': {
            const parts = [];
            if (args.user_sys_id)
                parts.push(`user=${args.user_sys_id}`);
            if (args.notification_sys_id)
                parts.push(`notification=${args.notification_sys_id}`);
            return await client.queryRecords({
        ...
        case 'snow_ntf_emergency_broadcast_send': {
            requireWrite();
        ...
        case 'snow_ntf_notification_schedule': {
            requireWrite();
