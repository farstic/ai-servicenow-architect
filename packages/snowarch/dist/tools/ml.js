...
export function mlToolManifest() {
    return [
        ...
        },
        {
            ...
            description: 'Find similar past incidents using keyword-based matching. Provide either an incident sys_id ' +
        ...
        },
        {
            ...
            description: 'Auto-categorize a record based on its description by analysing resolved records of the same table. ' +
...
export async function dispatchMlAction(client, name, args) {
    switch (name) {
        case 'snow_ml_change_risk_predict': {
            ...
            const highRisk = resp.records.filter((r) => r.risk === 'high' || r.risk === '1').length;
            return {
        ...
        case 'snow_ml_anomalies_detect': {
            if (!args.table || !args.field)
                throw new ServiceNowError('table and field are required', 'INVALID_REQUEST');
            ...
            const values = resp.records.map((r) => parseFloat(r[args.field]) || 0);
            const mean = values.reduce((a, b) => a + b, 0) / (values.length || 1);
            const variance = values.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) / (values.length || 1);
            const stdDev = Math.sqrt(variance);
            const threshold = args.threshold || 2;
            const anomalies = resp.records.filter((r) => Math.abs((parseFloat(r[args.field]) || 0) - mean) > threshold * stdDev);
        ...
        case 'snow_ml_incident_classifier_train': {
            ...
            if (solutions.count === 0)
                return { error: 'No classification ML solution found. Enable Predictive Intelligence.' };
            ...
            try {
                ...
                return { action: 'training_failed', solution: sol.name, error: err instanceof Error ? err.message : String(err) };
            }
        }
        case 'snow_ml_change_risk_train': {
            ...
            if (solutions.count === 0)
                return { error: 'No change risk ML solution found.' };
            const sol = solutions.records[0];
            try {
                await client.callNowAssist(`/api/now/ml/solution/${sol.sys_id}/train`, {});
                return { action: 'training_triggered', solution: sol.name, sys_id: sol.sys_id };
                ...
                return { action: 'training_failed', error: err instanceof Error ? err.message : String(err) };
            }
        }
        ...
        case 'snow_ml_model_evaluate': {
            if (!args.model_sys_id)
                throw new ServiceNowError('model_sys_id is required', 'INVALID_REQUEST');
        ...
        case 'snow_ml_virtual_agent_nlu_exec': {
            ...
            const since = new Date(Date.now() - days * 86400000).toISOString().slice(0, 19).replace('T', ' ');
            let query = `sys_created_on>=${since}`;
            if (args.topic_sys_id)
                query += `^topic=${args.topic_sys_id}`;
            ...
            const completed = conversations.records.filter((r) => r.state === 'completed' || r.state === 'resolved').length;
        ...
        case 'snow_ml_process_optimization_read': {
            ...
            const durations = resp.records.map((r) => { const c = new Date(r.sys_created_on).getTime(); const re = new Date(r.resolved_at).getTime(); return (re - c) / 3600000; }).filter((d) => d > 0);
            const avgDuration = durations.length > 0 ? durations.reduce((a, b) => a + b, 0) / durations.length : 0;
            const avgReassign = resp.records.reduce((sum, r) => sum + (parseInt(r.reassignment_count) || 0), 0) / (resp.count || 1);
        ...
        case 'snow_ml_similar_incidents_query': {
            ...
            const likeConditions = topKeywords.map((kw) => `short_descriptionLIKE${kw}`).join('^OR');
            ...
            const ranked = resp.records
                ...
                const matchCount = topKeywords.filter((kw) => desc.includes(kw)).length;
        ...
        case 'snow_ml_auto_categorize': {
            ...
            for (const r of resp.records) {
                ...
                if (!cat)
                    continue;
                if (!categoryData[cat])
                    categoryData[cat] = { count: 0, keywords: {} };
