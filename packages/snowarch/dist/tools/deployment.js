...
export async function dispatchDeploymentAction(client, name, args) {
    switch (name) {
        case 'snow_deploy_artifact_query': {
            if (!args.name)
                throw new ServiceNowError('name is required', 'INVALID_REQUEST');
            const tableMap = { business_rule: 'sys_script', script_include: 'sys_script_include', client_script: 'sys_script_client', ui_policy: 'sys_ui_policy', ui_action: 'sys_ui_action', widget: 'sp_widget', flow: 'sys_hub_flow', sys_properties: 'sys_properties' };
            const table = args.type ? (tableMap[args.type] || args.type) : 'sys_metadata';
            let query = `nameLIKE${args.name}`;
            if (args.scope)
                query += `^sys_scope.name=${args.scope}`;
        ...
        case 'snow_deploy_artifact_validate': {
            ...
            if (script.includes('current.update()'))
                issues.push('WARN: current.update() in script can cause infinite loops');
            if (script.includes('eval('))
                issues.push('SECURITY: eval() usage detected');
            if (script.includes('GlideRecord') && !script.includes('addQuery'))
                issues.push('PERF: GlideRecord without query filter may scan entire table');
            if (script.includes('gs.sleep'))
                issues.push('PERF: gs.sleep() blocks thread');
            if (record.active === 'false')
                issues.push('INFO: Artifact is inactive');
        ...
        case 'snow_deploy_artifact_clone': {
            requireScripting();
            if (!args.table || !args.sys_id || !args.new_name)
                throw new ServiceNowError('table, sys_id, and new_name are required', 'INVALID_REQUEST');
            ...
            if (args.target_scope)
                cloneData.sys_scope = args.target_scope;
        ...
        case 'snow_deploy_deployment_validate': {
            if (!args.update_set_sys_id && !args.app_sys_id)
                throw new ServiceNowError('update_set_sys_id or app_sys_id required', 'INVALID_REQUEST');
        ...
        case 'snow_deploy_deployment_rollback': {
            requireWrite();
            if (!args.update_set_sys_id)
        ...
        case 'snow_deploy_deployment_history_index': {
        ...
        case 'snow_deploy_solution_package_add': {
            requireWrite();
            if (!args.name || !args.update_sets?.length)
                throw new ServiceNowError('name and update_sets are required', 'INVALID_REQUEST');
        ...
        case 'snow_deploy_background_script_exec': {
            ...
            throw new ServiceNowError('Server-side script execution has no supported REST endpoint on this instance. Run the '
                + 'script in System Definition > Scripts - Background, or author it as a Fix Script '
                + '(sys_script_fix) and run it from the UI.', 'UNSUPPORTED_ON_THIS_INSTANCE');
        }
        case 'snow_deploy_cmdb_data_import': {
            requireWrite();
            if (!args.table || !args.data?.length)
                throw new ServiceNowError('table and data are required', 'INVALID_REQUEST');
            const results = [];
        ...
        case 'snow_deploy_data_quality_analyze': {
            if (!args.table)
