...
import { EXIT_INTERRUPTED, EXIT_USAGE, readSecretFromStdin } from './tty.js';
 ...
 * not have to guess.
 */
export const maskUsername = (username) => (username.length === 0 ? '' : `${username.slice(0, 1)}***`);
...
export function probeSummary(probe, flags, noProbes) {
    ...
    const field = { WRITE_ENABLED: 'write', CMDB_WRITE_ENABLED: 'cmdb', SCRIPTING_ENABLED: 'scripting',
        ATF_ENABLED: 'atf', NOW_ASSIST_ENABLED: 'nowAssist', FLUENT_ENABLED: 'fluent' };
...
export async function runAdd(options, io, deps = {}) {
    ...
    for (const note of normalised.notes)
        io.write(`  ${note}\n`);
    ...
    if (!reach.ok) {
        ...
        if (!again.ok) {
            for (const line of formatFailure(again))
                io.write(`${line}\n`);
            io.write(`${NOTHING_SAVED}\n`);
            return { saved: false, exitCode: EXIT_FAILED, message: NOTHING_SAVED };
        }
