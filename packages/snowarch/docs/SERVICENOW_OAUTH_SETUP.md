...
This guide walks you through creating an OAuth 2.0 application profile in ServiceNow for use with ServiceNow MCP Toolkit.
...
3. Select **Create an OAuth API endpoint for external clients**
   ...
   This option allows external applications (like ServiceNow MCP Toolkit) to authenticate to ServiceNow.
...
|-------|-------|-------------|
| **Name** | `ServiceNow MCP Toolkit` | Descriptive name for your OAuth app |
...
For ServiceNow MCP Toolkit, you can leave this blank or set to:
```
...
2. **IMPORTANT**: Copy the following values immediately:
   - **Client ID**: A long UUID-like string (e.g., `YOUR_CLIENT_ID`)
   - **Client Secret**: A long secret string (e.g., `YOUR_CLIENT_SECRET`)
...
Add the OAuth credentials to your ServiceNow MCP Toolkit `.env` file:
...
Most ServiceNow instances use the default `useraccount` scope which provides appropriate access for ServiceNow MCP Toolkit.
...
Create a dedicated ServiceNow service account for ServiceNow MCP Toolkit:
...
This limits ServiceNow MCP Toolkit to read operations only, reducing security risk.
