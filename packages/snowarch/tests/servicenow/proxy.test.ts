...
describe('criterion 1 - HTTPS_PROXY routes the request through the proxy', () => {
  it('the proxy records CONNECT <host>:443', async () => {
    ...
    await tryFetch('https://example.invalid/api/now/table/sys_user');
  ...
  it('NO_PROXY for that host bypasses it, and the proxy records nothing', async () => {
    vi.stubEnv('HTTPS_PROXY', `http://127.0.0.1:${port}`);
    vi.stubEnv('NO_PROXY', 'example.invalid');
    resetHttpDispatcher();

    const err = await tryFetch('https://example.invalid/api/now/table/sys_user');

    expect(connects).toEqual([]);
    // And it really tried: a bypassed request fails at DNS, which is the proof it went direct
    // rather than the proof that nothing happened.
