...
- After authoring or updating any SKILL.md **or EXAMPLES.md**, run a doc-verification pass against the relevant `ServiceNowDocs/markdown/` subfolder before committing. This is now enforced automatically by `node scripts/docs.mjs verify` + `tests/skills-lint.test.mjs` via `.githooks/pre-commit`; a dead citation or structural-integrity break blocks the commit.
